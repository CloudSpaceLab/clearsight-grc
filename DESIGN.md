...
Program follow-up groups assessment expiry, missing assessments, source updates and open issues into compact rows. Each row names the affected population, expands its supporting reasons and links to the relevant Program section. Desktop actions align beside the reason; below 560px they stack below it with a 44px minimum target. Supporting information status does not establish vendor document or response freshness. Submitted-response tables combine score and concern under Assessment result; scoring coverage remains in response detail. Existing theme and focus tokens apply.
...
Vendor portfolio workload uses stored, currently readable linked findings and actions. Four cards identify loaded services, open findings, open actions and overdue actions; selection filters the accompanying finding list. Service classification and submitted-form scoring do not stand in for findings or action completion. Partial or failed link/record reads show unknown totals. Source ratings, owners, periods and target dates retain their attribution. Selected-vendor Overview puts this same actionable list before form collection and review. Terminal requests retain Superseded, Revoked or Cancelled labels and no current missing-answer prompt.
...
## Vendor renewal — 25 September 2026

Program overview adds a compact, scoped vendor follow-up summary. It counts affected vendors distinctly, separates expired documents and response fields from internal review findings, and discloses vendor links on demand. Completed vendor reviews start a successor reassessment with expired fields preselected where the active form still contains them; sending remains a distinct step. Open requests offer an explicit email reminder with a new secure link. The design uses existing surfaces and controls, with no new token or density mode. See [decision brief](docs/design/2026-09-25-vendor-renewal-and-demo-curation.md).
