...
async function captureFieldVisit() {
  ...
  try {
    ...
    const notes = page.getByRole("textbox", { name: "Anything the reviewer should know?" });
    if (await notes.count() !== 1) throw new Error("Field visit does not expose exactly one optional exception note");
    await page.getByRole("radio", { name: "Yes" }).nth(0).check();
    await page.getByRole("radio", { name: "Yes" }).nth(1).check();
    await page.getByLabel(/Site photo/).setInputFiles({ name: "atm-site.jpg", mimeType: "image/jpeg", buffer: Buffer.from("field-visit-photo") });
    await page.getByText(/atm-site\.jpg/).waitFor();
    await page.locator(".file-dropzone-preview").waitFor({ state: "visible" });
...
async function captureImportSelection() {
  ...
  try {
    ...
    await page.getByLabel("Document").setInputFiles({ name: "outsourcing-policy.pdf", mimeType: "application/pdf", buffer: Buffer.from("sample-policy") });
