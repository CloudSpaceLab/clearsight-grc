...
try {
  ...
  await assertLifecycleContent(page, browserErrors);
  ...
  await capture(page, "33-today-lifecycle-collapsed-light-1440x900", "lifecycle-work-collapsed");

  await detail.locator("summary").click();
  await assertExpandedVerification(page);
  await assertNoHorizontalOverflow(page, "34-today-lifecycle-expanded-light-1440x900");
  await capture(page, "34-today-lifecycle-expanded-light-1440x900", "lifecycle-work-expanded");

  await detail.locator("summary").click();
  await page.setViewportSize({ width: 390, height: 844 });
  if (await detail.evaluate((element) => element.open)) throw new Error("mobile verification detail should start collapsed");
  await assertNoHorizontalOverflow(page, "35-today-lifecycle-collapsed-light-mobile-390x844");
  await assertFirstActionVisible(page, 762, "35-today-lifecycle-collapsed-light-mobile-390x844");
  await capture(page, "35-today-lifecycle-collapsed-light-mobile-390x844", "lifecycle-work-mobile-collapsed");

  await detail.locator("summary").click();
  await assertExpandedVerification(page);
  await assertNoHorizontalOverflow(page, "36-today-lifecycle-expanded-light-mobile-390x844");
  await capture(page, "36-today-lifecycle-expanded-light-mobile-390x844", "lifecycle-work-mobile-expanded");
...
async function assertExpandedVerification(page) {
...
async function assertFirstActionVisible(page, viewportBottom, name) {
  const action = page.locator(".intervention-next .primary-button").first();
  await action.waitFor({ state: "visible" });
  const box = await action.boundingBox();
  if (!box || box.y < 0 || box.y + box.height > viewportBottom) throw new Error(`${name} does not keep the first Today action inside the unobstructed initial viewport`);
}
...
async function appendRecord(page, name, state) {
  ...
  manifest.captures.push({ name, route: "#today", fixture: "today-lifecycle", state, viewport, theme: "light", density: "comfortable", metrics: await layoutMetrics(page) });
