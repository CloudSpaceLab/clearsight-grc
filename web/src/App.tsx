...
type View = "today" | "work" | "configure";
...
const fallbackGuide: OnboardingGuide = { code: "control-assurance-first-run", role: "Control Assurance Lead", version: 1, title: "Review workspace setup", description: "A short guide to assigned work, approval routes, evidence requests and readiness status.", illustration: "guided-orbit", steps: [
...
function App() {
  ...
  const [evidenceState, setEvidenceState] = useState<"loading" | "live" | "unavailable">("loading");
  ...
  useEffect(() => {
    Promise.allSettled([loadToday(), loadReadiness(), loadPolicies(), loadIntegrity(), loadWorkflowTasks(), loadEvidenceSources(), loadEvidenceRequests(), loadOnboardingGuide(), loadOnboardingState()]).then((results) => {
      const [todayResult, readinessResult, policiesResult, integrityResult, tasksResult, sourcesResult, requestsResult, guideResult, stateResult] = results;
  ...
  return <div className="app-shell">
    <aside className="sidebar" aria-label="Primary navigation">
      ...
      <nav>{["Today", "Programs", "Work", "Explore", "Configure"].map((label) => {
        const view: View | null = label === "Today" ? "today" : label === "Work" ? "work" : label === "Configure" ? "configure" : null;
    ...
    <main>
      ...
      {activeView === "work" && <WorkView sources={sources} requests={evidenceRequests} state={evidenceState}/>} 
...
function TodayView({ items, dueSoon, connection, readiness, onRouting, onCapture }: { items: AttentionItem[]; dueSoon: number; connection: string; readiness: Readiness | null; onRouting: () => void; onCapture: () => void }) {
  ...
  return <>
    ...
    <section className="quiet-section"><div><span className="verified-dot"/> No material changes detected in 6 demonstration programs</div><p>Evidence and source-check results are available in each program.</p></section>
  </>;
}

function WorkView({ sources, requests, state }: { sources: EvidenceSource[]; requests: EvidenceRequest[]; state: "loading" | "live" | "unavailable" }) {
  return <><header className="topbar"><div><span className="eyebrow">Demonstration Bank Nigeria · Evidence Operations</span><h1>Work</h1><p>Source health, evidence requests and response deadlines.</p></div></header><EvidenceWorkspace sources={sources} requests={requests} state={state}/></>;
}
