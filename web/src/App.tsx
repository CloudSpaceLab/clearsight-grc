...
import { loadCaptureRequest, loadContext, loadEvidenceRequests, loadEvidenceSources, loadIntegrity, loadOnboardingGuide, loadOnboardingState, loadPolicies, loadProjectionHealth, loadReadiness, loadToday, loadWorkflowTasks, reconcileProgramState, resolveAuthority, saveOnboardingState, submitCaptureRequest } from "./api";
...
function ProgramsView({ organizationName }: { organizationName: string }) {
...
function ExploreView({ organizationName }: { organizationName: string }) {
...
function WorkView({ organizationName, tab, onTab, sources, requests, evidenceState, onEvidenceRetry }: { organizationName: string; tab: "matters" | "evidence"; onTab: (value: "matters" | "evidence") => void; sources: EvidenceSource[]; requests: EvidenceRequest[]; evidenceState: LoadState; onEvidenceRetry: () => void }) {
...
function ConfigureView({ policies, findings, tasks, projectionHealth, state, onRetry, onReconcile }: { policies: PolicySummary[]; findings: IntegrityFinding[]; tasks: WorkflowTask[]; projectionHealth: ProjectionHealth | null; state: LoadState; onRetry: () => void; onReconcile: () => Promise<ReconcileResult> }) {
