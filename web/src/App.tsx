...
function App({ presentation = "enterprise" }: { presentation?: RuntimePresentation }) {
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {activeView === "ropa" && <section className="workspace-loading" aria-live="polite"><div><h1>{target.ropaPage === "reports" ? "Processing activity reports" : target.ropaActivityID ? "Processing activity" : "Processing activity register"}</h1><p>{target.ropaPage === "reports" ? "Processing activity reports are not yet available in this workspace. Return here when reporting is enabled." : target.ropaActivityID ? "The selected processing activity is not yet available in this workspace. Return to the register when the activity view is enabled." : "The processing activity register is not yet available in this workspace. Return here when the register view is enabled."}</p></div></section>}
