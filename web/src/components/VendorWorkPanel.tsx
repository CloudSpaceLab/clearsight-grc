...
function VendorWorkCard({ work, form, relationship, captureURL, onOpenRequest, onChanged }: { work: VendorWorkRequest; form?: FormTemplate; relationship: VendorRelationshipAggregate | null; captureURL?: string; onOpenRequest?: (requestID: string) => void; onChanged: (work: VendorWorkRequest, outcome?: VendorWorkSendOutcome) => void }) {
  ...
  const [responseAccessDenied, setResponseAccessDenied] = useState(false);
  ...
  const canReissue = retry || (work.delivery_state === "DELIVERED" && work.state === "AWAITING_VENDOR");
  ...
  async function run(action: () => Promise<VendorWorkRequest | VendorWorkSendOutcome>) {
    ...
    try {
      ...
      const recipientMismatch = caught instanceof ApiError && caught.code === "vendor_work_recipient_mismatch";
      const replacementMismatch = caught instanceof ApiError && caught.code === "vendor_work_replacement_mismatch";
      setError(acceptanceConflict ? "A submitted document cannot be used yet. Wait for inspection or request a replacement before accepting." : recipientMismatch ? "Use the contact that received the original request. Create a new request if the vendor contact has changed." : replacementMismatch ? "A replacement request was already prepared with different details. Restore the original instruction, recipient and deadline, then send it again." : kind === "conflict" ? "This request changed. Reload vendor requests before trying again." : kind === "forbidden" || kind === "unauthorized" ? "Your current access does not allow this action." : "The request could not be updated. Your entries remain on this screen.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function openResponse() {
    ...
    try {
      ...
      if (activeResponseIdentity.current !== requestedIdentity) return;
      setResponseView(undefined);
      setResponseAccessDenied(apiErrorKind(caught) === "forbidden");
  ...
  return <article className="vendor-work-card" data-testid={`vendor-work-${work.id}`}>
    ...
    {canReissue && <div className="vendor-work-action"><div className="vendor-work-contact"><label>{requestRecipientLabel(work.request_kind)}<input type="email" autoComplete="email" aria-describedby={`vendor-work-contact-help-${work.id}`} value={audience} onChange={(event) => setAudience(event.target.value)}/></label><small id={`vendor-work-contact-help-${work.id}`}>Use the same contact as the original request. Create a new request if this contact has changed.</small></div><button type="button" className="primary-button" disabled={busy || !validEmail(audience)} onClick={() => void run(() => retryVendorWorkDelivery(work.relationship_id, work.id, { expected_version: work.version, vendor_audience: audience.trim(), invitation_ttl_minutes: invitationTTLMinutes }))}>{busy ? setupIncomplete ? "Completing…" : retry ? "Retrying…" : "Sending again…" : setupIncomplete ? "Complete setup" : retry ? "Retry delivery" : "Send again"}</button></div>}
    {!canReissue && work.state === "PREPARING" && <div className="vendor-work-action"><label>{requestRecipientLabel(work.request_kind)}<input type="email" autoComplete="email" value={audience} onChange={(event) => setAudience(event.target.value)}/></label><button type="button" className="primary-button" disabled={busy || !validEmail(audience)} onClick={() => void run(() => sendVendorWork(work.relationship_id, work.id, { expected_version: work.version, vendor_audience: audience.trim(), invitation_ttl_minutes: invitationTTLMinutes }))}>{busy ? "Sending…" : requestSendLabel(work.request_kind)}</button></div>}
    ...
    {responseState === "failed" && <div role="alert" className="inline-error"><p>{responseAccessDenied ? "Reviewer access is required. Switch to the Internal Auditor demo account to review the submitted response." : "The submitted response could not be loaded. Review has not started."}</p>{!responseAccessDenied && <button type="button" className="secondary-button" onClick={() => void openResponse()}>Try again</button>}</div>}
    ...
    {work.state === "UNDER_REVIEW" && responseState === "ready" && !mode && <div className="vendor-work-buttons"><button type="button" className="primary-button" disabled={acceptanceBlocked} onClick={() => setMode("accept")}>Accept response</button><button type="button" className="secondary-button" onClick={() => { setDueDate(work.due_at.slice(0, 10)); setMode("changes"); }}>Request changes</button></div>}
