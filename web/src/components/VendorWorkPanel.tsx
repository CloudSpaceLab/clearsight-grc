...
export function VendorWorkPanel(props: Props) {
  ...
  if (state === "loading") return <section className="vendor-work-panel" tabIndex={-1} aria-live="polite" aria-busy="true">Loading vendor requests for this {targetName}…</section>;
  if (state === "failed") return <section className="vendor-work-panel" tabIndex={-1} role="alert"><h2>Vendor requests</h2><p>Vendor requests could not be loaded for this {targetName}. No request state is shown.</p><button type="button" className="secondary-button" onClick={() => void loadAll()}>Try again</button></section>;

  return <section className="vendor-work-panel" tabIndex={-1} aria-labelledby={headingID}>
