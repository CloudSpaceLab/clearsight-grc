...
import { Button, CheckboxField, FocusedSheet, Notice, SelectField, TextArea, TextField } from "./ui";
...
export function VendorWorkPanel(props: Props) {
  ...
  if (state === "failed") return <section className="vendor-work-panel" data-testid={relationshipID ? `vendor-work-relationship-${relationshipID}` : `vendor-work-${targetType}-${targetID}`} tabIndex={-1} role="alert"><h2>Vendor requests</h2><p>Vendor requests could not be loaded for this {targetName}. No request state is shown.</p><Button type="button" variant="secondary" onPress={() => void loadAll()}>Try again</Button></section>;
  ...
  return <section className="vendor-work-panel" data-testid={relationshipID ? `vendor-work-relationship-${relationshipID}` : `vendor-work-${targetType}-${targetID}`} tabIndex={-1} aria-labelledby={headingID}>
    ...
    {nextCursor && <div className="vendor-work-load-more"><Button type="button" variant="secondary" isDisabled={loadingMore} onPress={() => void loadMore()}>{loadingMore ? "Loading…" : "Load more vendor requests"}</Button></div>}
    ...
    {creating && <FocusedSheet label="Request vendor work" closeLabel={saving ? "Vendor request is being sent" : "Close vendor request"} size="wide" panelClassName="vendor-work-request-sheet" isDismissable={!saving} onClose={() => { if (saving) return; setCreating(false); resetDraft(); }}>
      ...
      <form className="vendor-work-form" onSubmit={(event) => void prepareAndSend(event)}>
        ...
        {requestKind === "CERTIFICATION_REFRESH" && <Notice tone="info">The vendor is being asked for current ISO 27001 and PCI DSS evidence. Submission does not mean it was accepted; the assigned reviewer must check and accept the evidence.</Notice>}
        ...
        <TextArea label="Instructions for the vendor" value={instructions} onChange={setInstructions} maxLength={2000} rows={4} isRequired placeholder="State what must be completed or provided."/>
...
function VendorWorkCard({ work, form, relationship, captureURL, onOpenRequest, onChanged }: { work: VendorWorkRequest; form?: FormTemplate; relationship: VendorRelationshipAggregate | null; captureURL?: string; onOpenRequest?: (requestID: string) => void; onChanged: (work: VendorWorkRequest, outcome?: VendorWorkSendOutcome) => void }) {
  ...
  return <article className="vendor-work-card" data-testid={`vendor-work-${work.id}`}>
    ...
    {captureURL && <div className="vendor-work-secure-link"><TextField label="Secure link" isReadOnly value={captureURL} onChange={() => undefined} onFocus={(event) => event.currentTarget.select()}/><Button type="button" variant="secondary" onPress={() => void copySecureLink()}>Copy secure link</Button>{copyNotice && <small role="status">{copyNotice}</small>}</div>}
    {retry && <div className="vendor-work-action"><TextField label={requestRecipientLabel(work.request_kind)} type="email" autoComplete="email" value={audience} onChange={(value) => setAudience(value)}/><Button type="button" variant="primary" isDisabled={busy || !validEmail(audience)} onPress={() => void run(() => retryVendorWorkDelivery(work.relationship_id, work.id, { expected_version: work.version, vendor_audience: audience.trim(), invitation_ttl_minutes: invitationTTLMinutes }))}>{busy ? setupIncomplete ? "Completing…" : "Retrying…" : setupIncomplete ? "Complete setup" : "Retry delivery"}</Button></div>}
    {!retry && work.state === "PREPARING" && <div className="vendor-work-action"><TextField label={requestRecipientLabel(work.request_kind)} type="email" autoComplete="email" value={audience} onChange={(value) => setAudience(value)}/><Button type="button" variant="primary" isDisabled={busy || !validEmail(audience)} onPress={() => void run(() => sendVendorWork(work.relationship_id, work.id, { expected_version: work.version, vendor_audience: audience.trim(), invitation_ttl_minutes: invitationTTLMinutes }))}>{busy ? "Sending…" : requestSendLabel(work.request_kind)}</Button></div>}
    {work.current_request_id && onOpenRequest && <Button type="button" variant="secondary" onPress={() => onOpenRequest(work.current_request_id!)}>Open collection request</Button>}
    {(work.state === "RESPONSE_RECEIVED" || work.state === "UNDER_REVIEW") && responseState === "closed" && <Button type="button" variant="primary" isDisabled={!work.current_request_id} onPress={() => void openResponse()}>Review response</Button>}
    {responseState === "loading" && <p aria-live="polite" aria-busy="true">Loading the submitted response…</p>}
    {responseState === "failed" && <div role="alert" className="inline-error"><p>The submitted response could not be loaded. Review has not started.</p><Button type="button" variant="secondary" onPress={() => void openResponse()}>Try again</Button></div>}
    {responseView && responseState === "ready" && <VendorWorkResponseReview view={responseView}/>}
    {work.state === "RESPONSE_RECEIVED" && responseState === "ready" && <Button type="button" variant="primary" isDisabled={busy} onPress={() => void beginReview()}>{busy ? "Starting review…" : "Begin review"}</Button>}
    ...
    {work.state === "UNDER_REVIEW" && responseState === "ready" && !mode && <div className="vendor-work-buttons"><Button type="button" variant="primary" isDisabled={acceptanceBlocked} onPress={() => setMode("accept")}>Accept response</Button><Button type="button" variant="secondary" onPress={() => setMode("changes")}>Request changes</Button></div>}
    {mode === "accept" && <div className="vendor-work-decision"><TextArea label="Acceptance basis" rows={3} maxLength={2000} value={rationale} onChange={(value) => setRationale(value)}/><div className="form-actions"><Button type="button" variant="secondary" onPress={() => setMode(null)}>Back</Button><Button type="button" variant="primary" isDisabled={busy || !rationale.trim()} onPress={() => void run(() => acceptVendorWork(work.relationship_id, work.id, { expected_version: work.version, rationale: rationale.trim() }))}>{busy ? "Accepting…" : "Confirm acceptance"}</Button></div></div>}
    {mode === "changes" && <div className="vendor-work-decision"><TextArea label={changeInstructionLabel(work.request_kind)} rows={3} maxLength={2000} value={message} onChange={(value) => setMessage(value)}/>{changeFields.length ? <fieldset><legend>Answers or documents to update</legend>{changeFields.map((field) => <CheckboxField key={field.id} label={field.label} isSelected={fieldIDs.includes(field.id)} onChange={(selected) => setFieldIDs((current) => selected ? [...current, field.id] : current.filter((id) => id !== field.id))}/>)}</fieldset> : <p className="inline-notice">The form fields could not be loaded. Reload vendor requests before requesting changes.</p>}<div className="vendor-work-form-grid"><TextField label={requestRecipientLabel(work.request_kind)} type="email" value={audience} onChange={(value) => setAudience(value)}/><TextField label="Revised due date" type="date" min={minimumDueDate()} value={dueDate} onChange={(value) => setDueDate(value)}/></div><div className="form-actions"><Button type="button" variant="secondary" onPress={() => setMode(null)}>Back</Button><Button type="button" variant="primary" isDisabled={busy || !message.trim() || !fieldIDs.length || !validEmail(audience) || !dueDate} onPress={() => void run(() => requestVendorWorkChanges(work.relationship_id, work.id, { expected_version: work.version, message: message.trim(), field_ids: fieldIDs, vendor_audience: audience.trim(), due_at: endOfDayUTC(dueDate), invitation_ttl_minutes: invitationTTLMinutes }))}>{busy ? "Sending changes…" : "Send change request"}</Button></div></div>}
    {mode === "cancel" && <div className="vendor-work-decision"><TextArea label="Cancellation reason" rows={3} maxLength={1000} value={rationale} onChange={(value) => setRationale(value)}/><div className="form-actions"><Button type="button" variant="secondary" onPress={() => setMode(null)}>Back</Button><Button type="button" variant="primary" isDisabled={busy || !rationale.trim()} onPress={() => void run(() => cancelVendorWork(work.relationship_id, work.id, { expected_version: work.version, reason: rationale.trim() }))}>{busy ? "Cancelling…" : "Cancel request"}</Button></div></div>}
    ...
    {canCancel && mode !== "cancel" && <div className="vendor-work-cancel"><Button type="button" variant="quiet" isDisabled={busy} onPress={() => setMode("cancel")}>Cancel request</Button></div>}
    {error && <p role="alert" className="inline-error">{error}</p>}
  </article>;
}

function VendorWorkResponseReview({ view }: { view: VendorWorkResponseView }) {
  const responseLabel = "Vendor response";
...
function evidenceClassLabel(value: string) { if (value === "VENDOR_SUPPLIED") return "Vendor supplied"; if (value === "STAFF_SUPPLIED") return "Assigned staff supplied"; if (value === "BANK_VALIDATED") return "Validated by reviewer"; if (value === "OFFICIAL_SOURCE") return "Official source"; return documentTypeLabel(value); }
...
function requestRecipientLabel(kind: VendorWorkRequestKind | undefined) { return kind === "CERTIFICATION_REFRESH" ? "Vendor contact email" : "Vendor contact"; }
function changeInstructionLabel(_kind: VendorWorkRequestKind | undefined) { return "What the vendor must change"; }
function requestSendLabel(kind: VendorWorkRequestKind | undefined) { return kind === "CERTIFICATION_REFRESH" ? "Send certification request" : "Prepare and send request"; }
