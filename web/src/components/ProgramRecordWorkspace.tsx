...
export function ProgramRecordWorkspace({ programID, onBack, actorPrincipalID = "", canConfigureSources = false, onOpenMatter = (matterID) => { window.location.hash = `#work/matters/${encodeURIComponent(matterID)}`; }, onOpenRequest }: Props) {
  ...
  return <section className="program-record-workspace" aria-label="Program record">
    ...
    {aggregate && <>
      ...
      {operationsState === "live" && operations?.responsibility_labels_complete === false && <div className="inline-notice" role="status"><strong>Some assignee names could not be loaded.</strong> Recorded Program responsibilities remain visible, and available actions still use the current authority route. <button className="text-button" type="button" onClick={() => void loadOperations()}>Reload assignee names</button></div>}
