...
import { ProgramVendorFollowUp } from "./ProgramVendorFollowUp";
...
export function ProgramRecordWorkspace({ programID, section = "overview", programItem, onSectionChange, onBack, actorPrincipalID = "", canConfigureSources = false, onOpenMatter = (matterID) => { window.location.hash = `#work/matters/${encodeURIComponent(matterID)}`; }, onOpenRequest, onOpenForm }: Props) {
  ...
  return <section className="program-record-workspace" aria-label="Program record">
    ...
    {aggregate && <>
      ...
      {operationsState === "live" && operations?.responsibility_labels_complete === false && <div className="inline-notice" role="status"><strong>Some assignee names could not be loaded.</strong> Recorded Program assignments remain visible, and available actions continue to use the current assignments. <button className="text-button" type="button" onClick={() => void loadOperations()}>Reload assignee names</button></div>}
      ...
      {activeSection === "overview" && <ProgramVendorFollowUp programID={aggregate.program.id}/>}
