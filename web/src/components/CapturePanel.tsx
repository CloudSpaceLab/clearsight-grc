...
export function CapturePanel({ request, state = "live", onReload, external = false, sessionToken, onSubmit, onUploadArtifact }: Props) {
  ...
  if (reviewing) return <CaptureReview request={request} fields={visibleCaptureFields(contract, answers)} answers={answers} attachments={attachments} submitting={submitting} error={error} errorKind={errorKind} external={external} onEdit={() => setReviewing(false)} onReload={onReload} onSubmit={() => void submit()}/>;
