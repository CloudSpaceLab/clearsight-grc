...
export function ProgramStatusPanel({ aggregate, operations, onUpdated, onReload }: Props) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const value = await transitionProgram(aggregate.program.id, aggregate.program.version, selectedTarget, rationale.trim());
  ...
  return <article className="program-record-panel" id="program-status-panel">
    ...
    {operation?.can_act && targets.length > 0 ? <form className="program-operation-form" onSubmit={(event) => void submit(event)}>
      <label><span>New operating status</span><select value={selectedTarget} onChange={(event) => setTarget(event.target.value)}>{targets.map((value) => <option value={value} key={value}>{label(value)}</option>)}</select></label>
      ...
      <div className="program-form-actions wide"><button className="primary-button" disabled={busy || !selectedTarget || !rationale.trim()} type="submit">{busy ? "Saving…" : action(selectedTarget)}</button></div>
