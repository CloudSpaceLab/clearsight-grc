...
export function EvidenceWorkspace({ sources, requests, state, targetID, openFirst = false, onOpenRequest }: Props) {
  ...
  const targetMissing = Boolean(state === "live" && targetID && !requests.some((request) => request.id === targetID));
  ...
  if (state === "loading" && !requests.length && !sources.length) {
    return <section id="evidence-workspace" className="workspace-loading" aria-live="polite" aria-busy="true">Loading evidence work…</section>;
  }

  return <div id="evidence-workspace">
    <section className="workspace-brief">
      <div><span className="eyebrow">Evidence review</span><h2>{state === "unavailable" ? "Evidence work could not be loaded" : state === "loading" ? "Refreshing evidence work" : `${openRequests} open evidence request${openRequests === 1 ? "" : "s"}`}</h2><p>Human requests and review work stay primary; the complete source inventory is available only when you need deeper context.</p></div>
    ...
    {targetMissing && <EmptyState label="Requested evidence request" title="The requested evidence request could not be loaded" description="It may be outside your current access scope or no longer available."/>}
