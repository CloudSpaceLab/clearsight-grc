  ...
  return <article className="program-record-panel program-evidence-panel" id="program-evidence-panel">
    ...
    {aggregate.evidence_contracts.length ? <div className="program-evidence-list">{aggregate.evidence_contracts.map((contract) => {
      ...
      return <section className="program-evidence-card" key={contract.id}>
        ...
        <div className={latest ? "program-assessment-result" : "program-assessment-result empty"}>{latest ? <><strong>{statusLabel(latest.conclusion)}</strong><span>{Math.round(latest.coverage * 100)}% coverage · {current ? `Program assessment valid until ${latest.valid_until!.slice(0, 10)}` : latest.valid_until ? `Program assessment expired ${latest.valid_until.slice(0, 10)}` : "assessment validity not recorded"}</span><small>Assessed {latest.assessed_at.slice(0, 10)} by {reviewerLabel(latest.id, latest.assessed_by)}</small></> : <><strong>No Program assessment recorded</strong><span>A reviewer must assess the supporting source before this check can support the Program outcome.</span></>}</div>
