  ...
  return <article className="program-record-panel program-evidence-panel" id="program-evidence-panel">
    ...
    {aggregate.evidence_contracts.length ? <div className="program-evidence-list">{aggregate.evidence_contracts.map((contract) => {
      ...
      return <section className="program-evidence-card" key={contract.id}>
        ...
        <div className={latest ? "program-assessment-result" : "program-assessment-result empty"}>{latest ? <><strong>{statusLabel(latest.conclusion)}</strong><span>{Math.round(latest.coverage * 100)}% coverage · {current ? `current until ${latest.valid_until!.slice(0, 10)}` : latest.valid_until ? `expired ${latest.valid_until.slice(0, 10)}` : "validity not recorded"}</span><small>Assessed {latest.assessed_at.slice(0, 10)}{latest.assessed_by ? " · Reviewer name unavailable" : ""}</small></> : <><strong>No evidence result recorded</strong><span>A reviewer must assess the evidence before this check can support the Program outcome.</span></>}</div>
      </section>;
