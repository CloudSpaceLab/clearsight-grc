...
import type { EvidenceSource, ProgramAggregate } from "../types";
...
export function ProgramEvidencePanel({ aggregate, operations, actorPrincipalID, canConfigureSources, canOperate = true, onUpdated, onReload }: Props) {
  ...
  return <article className="program-record-panel program-evidence-panel" id="program-evidence-panel">
    ...
    {aggregate.evidence_contracts.length ? <div className="program-evidence-list">{aggregate.evidence_contracts.map((contract) => {
      ...
      const reviewerLabel = (reviewerID?: string, fallback = "Reviewer name unavailable") => operations.flatMap((operation) => operation.candidates ?? []).find((candidate) => candidate.id === reviewerID)?.display_name ?? fallback;
    ...
    {mode === "define" && <form className="program-operation-form" onSubmit={(event) => void saveDefinition(event)}>
      ...
      <label><span>Contradiction handling</span><select value={contradictionPolicy} onChange={(event) => setContradictionPolicy(event.target.value)}><option value="REVIEW">Require review</option><option value="FAIL">Fail the check</option><option value="FLAG">Flag for follow-up</option></select></label>
      <label><span>If the check fails</span><select value={failureAction} onChange={(event) => setFailureAction(event.target.value)}><option value="MATTER">Create an issue</option><option value="ESCALATE">Escalate</option><option value="REVIEW">Require review</option></select></label>
    ...
    {mode === "assess" && <form className="program-operation-form" onSubmit={(event) => void saveAssessment(event)}>
      ...
      <label><span>Conclusion</span><select value={conclusion} onChange={(event) => setConclusion(event.target.value)}><option value="SUPPORTED">Supported</option><option value="PARTIALLY_SUPPORTED">Partly supported</option><option value="NOT_SUPPORTED">Not supported</option><option value="INCONCLUSIVE">Inconclusive</option></select></label>
