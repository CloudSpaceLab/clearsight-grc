...
export function SystemOperationsSection({ canReconcile }: { canReconcile: boolean }) {
  ...
  return <section className="configure-domain" aria-labelledby="system-operations-heading">
    <header className="configure-domain-header">
      ...
      {view === "health" && state === "unavailable" && <button className="secondary-button" type="button" onClick={() => void loadHealth()}>Retry</button>}
    </header>

    <nav className="configure-subnav" aria-label="System operations views">
      <button type="button" className={view === "health" ? "active" : ""} aria-current={view === "health" ? "page" : undefined} onClick={() => setView("health")}>Health</button>
      <button type="button" className={view === "activity" ? "active" : ""} aria-current={view === "activity" ? "page" : undefined} onClick={() => setView("activity")}>Activity</button>
      <button type="button" className={view === "audit" ? "active" : ""} aria-current={view === "audit" ? "page" : undefined} onClick={() => setView("audit")}>Audit log</button>
    </nav>

    {view === "activity" && <SystemActivityPanel mode="activity" />}
    {view === "audit" && <SystemActivityPanel mode="audit" />}
    {view === "health" && <>
