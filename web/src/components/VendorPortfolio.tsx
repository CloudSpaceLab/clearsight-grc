    ...
    <div className="vendor-findings" ref={list}>
      ...
      <ul>{sorted.slice(0, limit).map(({ record, relationshipIDs }) => {
        ...
        return <li key={record.matter.id} className="vendor-finding">
          ...
          <div className="vendor-finding-source">{facts.sample === true && <span>Sample data</span>}{typeof facts.source_rating === "string" && facts.source_rating && <span>Source rating: {facts.source_rating}</span>}{typeof facts.source_owner === "string" && facts.source_owner && <span>Source owner: {facts.source_owner}</span>}{typeof facts.source_period === "string" && facts.source_period && <span>Source period: {facts.source_period}</span>}</div>
