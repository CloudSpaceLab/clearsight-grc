...
function PolicyDetail({ policy, simulation, rollbackTarget, busy, onAction, onRevise, forms }: { forms: PolicyFormChoice[]; onRevise: () => void; policy: FormResponsePolicy; simulation?: FormPolicySimulation; rollbackTarget?: FormResponsePolicy; busy: boolean; onAction: () => void }) {
  ...
  return <article className="forms-policy-detail" aria-labelledby={`policy-${policy.id}`}>
    ...
    <dl className="forms-policy-facts"><div><dt>Form scope</dt><dd>{matchingForm?.name ?? formName ?? "Form name unavailable"} · revision {policy.eligibility.form_template_version}</dd></div><div><dt>Result used</dt><dd>{policy.eligibility.result_basis === "BANK_ASSESSED" ? "Completed assessment" : "Automatic submission result"}</dd></div><div><dt>Concern threshold</dt><dd>{policy.eligibility.bands?.map(statusLabel).join(", ") || "Configured score threshold"}</dd></div><div><dt>Rollout</dt><dd>{policy.rollout === "SHADOW" ? "Record matches only" : "Create issues"}</dd></div><div><dt>New issue limits</dt><dd>{policy.blast_radius.per_run} per run · {policy.blast_radius.per_day} per day</dd></div><div><dt>Outcome check</dt><dd>After {policy.outcome_contract.check_after_minutes} minutes</dd></div><div><dt>Policy revision</dt><dd>{policy.version}</dd></div></dl>
