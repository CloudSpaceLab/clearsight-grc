...
export function FormPoliciesView() {
  ...
  return <section className="forms-policies" aria-labelledby="form-policies-title">
    ...
    {state === "live" && items.length === 0 && (
      <EmptyState population="Response policies in this legal entity" title="No response policies have been created" description="Create a draft to define a bounded response population, Matter handling and outcome check." action={<Button variant="primary" onPress={() => setCreating(true)}>Create policy</Button>}/>
    )}
...
function PolicyDetail({ policy, simulation, rollbackTarget, busy, onAction }: { policy: FormResponsePolicy; simulation?: FormPolicySimulation; rollbackTarget?: FormResponsePolicy; busy: boolean; onAction: () => void }) {
  ...
  return <article className="forms-policy-detail" aria-labelledby={`policy-${policy.id}`}>
    ...
    {policy.status === "SUSPENDED" && !rollbackTarget && <Notice tone="warning">No earlier revision of this policy is available for rollback. Create a new governed policy draft instead.</Notice>}
  </article>;
}
