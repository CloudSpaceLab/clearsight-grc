...
export function FormPolicyEditor({ onCancel, onCreate, forms, formsLoading = false, formsError = "", onRetryForms, automationChoices = [], automationLoading = false, automationError = "", onSelectForm, onRetryAutomation, initialInput, busy = false }: Props) {
  ...
  function submit(event: FormEvent) {
    ...
    if (!value.create_automation_policy && !automationChoices.some((choice) => choice.id === value.automation_policy_id && choice.version === value.automation_policy_version && choice.status === "ACTIVE")) { setError("Choose an active automation policy or create limits for this response policy."); return; }
  ...
  return <form className="forms-policy-editor" noValidate onSubmit={submit}>
    ...
    <section aria-labelledby="policy-population-title"><h3 id="policy-population-title">Responses covered</h3><p>Choose the approved form revision and the bank records whose responses this policy covers.</p><div className="forms-policy-control-grid">
    ...
    <section aria-labelledby="policy-automation-title"><h3 id="policy-automation-title">Automation limits</h3><div className="forms-policy-control-grid">
      ...
      {automationError && <><Notice tone="warning">{automationError} You can still save limits with this response policy.</Notice>{onRetryAutomation && <Button type="button" variant="quiet" onPress={onRetryAutomation}>Retry automation policies</Button>}</>}

      {!automationLoading && !automationError && automationChoices.length === 0 && <p>No automation policies were found for this form revision. Save limits with this response policy.</p>}
      ...
      <SelectField label="Rollout mode" value={value.rollout} placeholder="Choose rollout" allowsEmpty={false} options={[{ id: "SHADOW", label: "Record matches only" }, { id: "ENFORCE", label: "Create issues" }]} onChange={(rollout) => { if (rollout) patch("rollout", rollout as FormPolicyRollout); }}/>
    ...
    <section aria-labelledby="policy-matter-title"><h3 id="policy-matter-title">Issue and outcome</h3><div className="forms-policy-control-grid">
      ...
      <div className="forms-policy-control-grid__full"><TextArea label="What needs to happen next" value={value.action.requested_handling} onChange={(requested_handling) => patch("action", { ...value.action, requested_handling })} rows={2}/></div>
