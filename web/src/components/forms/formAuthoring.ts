...
import type {
  ...
  FormFieldType,
  FormTemplate as MonitoringFormTemplate,
  FormTemplateField,
  FormTemplateSection,
...
export function changeFieldType(field: AuthoringField, type: FormFieldType, scoringMode: FormScoringMode): AuthoringField {
  ...
  const next: AuthoringField = {
    ...
    constraints: initialConstraints(type),
    scoring: selectionType(type) && scoringMode !== "NONE" && field.scoring
...
export function enableScoring(field: AuthoringField, scoringMode: FormScoringMode): AuthoringField {
  ...
  return {
    ...field,
    scoring: scoringForOptions(field.scoring?.weight ?? 100, options, scoringMode, field.scoring),
  };
}

export function scoringForOptions(
  weight: number,
  options: string[],
  mode: FormScoringMode,
  previous?: FormTemplateField["scoring"],
): NonNullable<FormTemplateField["scoring"]> {
  ...
  return {
    weight: clamp(weight, 1, 100),
    answer_scores,
    critical_answers: (previous?.critical_answers ?? []).filter((answer) => options.includes(answer)),
  };
}

export function updateConstraint(
  field: AuthoringField,
  key: keyof CaptureFieldConstraints,
...
export function copySectionFromTemplate(
  ...
  if (!source) return undefined;
  return cloneSectionWithFields(
    source,
    template.fields.filter((field) => field.section_id === sectionID) as AuthoringField[],
    nextSection,
    nextField,
    false,
  );
...
function cloneSectionWithFields(
  source: AuthoringSection,
  sourceFields: AuthoringField[],
  nextSection: number,
  nextField: number,
  preserveExternalConditions: boolean,
) {
  ...
  const rewriteCondition = (condition?: CaptureVisibilityCondition): CaptureVisibilityCondition | undefined => {
    ...
    if (field_id) return cloneCondition({ ...condition, field_id });
    return preserveExternalConditions ? cloneCondition(condition) : undefined;
  };
  return {
    section: {
      ...cloneSection(source),
      id: newSectionID,
      title: preserveExternalConditions ? `${source.title} copy` : source.title,
      condition: rewriteCondition(source.condition),
    },
    fields: sourceFields.map((field) => {
      ...
      return {
        ...
        condition: rewriteCondition(field.condition),
        scoring,
      };
    }),
...
export function buildCreateInput(draft: FormDraft): CreateFormTemplateInput {
  return {
    ...
    sections: draft.sections.map((section) => ({
      ...
      ...(section.help?.trim() ? { help: section.help.trim() } : {}),
      ...(section.weight ? { weight: section.weight } : {}),
      ...(section.condition ? { condition: cleanCondition(section.condition) } : {}),
    })),
    fields: draft.fields.map((field) => cleanField(field, draft.scoringMode)),
  };
}
...
export function addRequiredSignOff(_fields: AuthoringField[], sectionID: string, nextField: number) {
...
function cleanField(field: AuthoringField, scoringMode: FormScoringMode): FormTemplateField {
  const options = selectionType(field.type)
    ? field.type === "yes_no" ? ["Yes", "No"] : uniqueOptions(field.options ?? [])
    : undefined;
  const scoring = field.scoring && options
    ? scoringForOptions(field.scoring.weight, options, scoringMode, field.scoring)
    : undefined;
  ...
  return {
    ...
    ...(field.record_target ? {
      record_target: {
        key: field.record_target.key.trim().toUpperCase(),
        required_subject_type: field.record_target.required_subject_type.trim().toUpperCase(),
      },
    } : {}),
...
function cloneSection(section: AuthoringSection): AuthoringSection {
  return { ...section, condition: cloneCondition(section.condition) };
}
...
function selectionType(type: FormFieldType) {
  return ["yes_no", "single_select", "multi_select"].includes(type);
}

export function fileType(type: FormFieldType) {
  return ["file", "photo", "signature", "vendor_document"].includes(type);
}

export function textType(type: FormFieldType) {
  return ["short_text", "long_text", "email", "telephone", "url"].includes(type);
}

export function numericType(type: FormFieldType) {
  return ["integer", "decimal", "percentage", "currency"].includes(type);
}

export function isSelectionType(type: FormFieldType) {
...
function initialConstraints(type: FormFieldType): CaptureFieldConstraints | undefined {
  ...
  if (["photo", "signature", "vendor_document", "file"].includes(type)) return { max_files: 1 };
  return undefined;
}
