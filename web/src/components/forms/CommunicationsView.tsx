...
import { Button, EmptyState, Notice, TextField } from "../ui";
...
export default function CommunicationsView() {
  ...
  return <section className="forms-task-shell" aria-labelledby="communications-title">
    ...
    <div className="forms-task-layout">
      ...
      <main className="forms-communication-detail">{selected ? <><span className="forms-detail-kicker">{selected.action} · {selected.locale}</span><h3>{selected.subject_template}</h3><p>v{selected.version} · {label(selected.status)} · effective {formatDate(selected.effective_from)}</p><div className="forms-inline-actions"><Button size="compact" isDisabled={Boolean(busy)} isLoading={busy === "preview"} onPress={() => void serverPreview()}>Preview message</Button><Button size="compact" isDisabled={Boolean(busy)} isLoading={busy === "impact"} onPress={() => void serverImpact()}>Check impact</Button><Button size="compact" onPress={() => setEditorOpen(true)}>Edit as new revision</Button></div>{preview && <section className="forms-server-preview" aria-label="Communication preview"><span>Preview</span><h4>{preview.subject}</h4><p>{preview.plain_text}</p></section>}{impact && <dl className="forms-impact"><div><dt>Subject changed</dt><dd>{yesNo(impact.subject_changed)}</dd></div><div><dt>Document changed</dt><dd>{yesNo(impact.document_changed)}</dd></div><div><dt>Window changed</dt><dd>{yesNo(impact.effective_window_changed)}</dd></div></dl>}<div className="forms-detail-actions">{selected.status === "DRAFT" && <Button variant="primary" isDisabled={Boolean(busy)} onPress={() => void templateTransition("PENDING_APPROVAL")}>Send for approval</Button>}{selected.status === "PENDING_APPROVAL" && <Button variant="primary" isDisabled={Boolean(busy)} onPress={() => void templateTransition("ACTIVE")}>Activate template</Button>}{selected.status === "ACTIVE" && <Button isDisabled={Boolean(busy)} onPress={() => void templateTransition("RETIRED")}>Retire template</Button>}<Button isDisabled={Boolean(busy)} onPress={() => void templateRollback()}>Create rollback draft</Button></div><div className="forms-test-send"><TextField label="Test recipient" type="email" value={testAddress} onChange={setTestAddress}/><Button variant="primary" isDisabled={Boolean(busy) || !testAddress.trim()} isLoading={busy === "test-send"} onPress={() => void testSend()}>Send test email</Button></div></> : <EmptyState population="Communication templates for this legal entity" title="No template is selected" description="Select an existing message template or create the first governed draft." action={<Button variant="primary" onPress={() => setEditorOpen(true)}>Create template</Button>}/>}</main>
...
function ProfileEditor({ initial, busy, onCancel, onSaved }: { initial?: CommunicationProfile; busy: boolean; onCancel: () => void; onSaved: () => Promise<void> }) {
  ...
  return <section className="forms-task-card"><div className="forms-task-heading"><div><span>New governed revision</span><h2>Communication profile</h2><p>Editing creates another immutable profile revision.</p></div><Button onPress={onCancel}>Close profile editor</Button></div>{error && <Notice tone="error">{error}</Notice>}<div className="forms-task-grid"><TextField label="Bank or organization name" value={bank} onChange={setBank}/><TextField label="Default locale" value={locale} onChange={setLocale}/><TextField label="Support contact" value={support} onChange={setSupport}/><TextField label="Inspected brand asset ID" value={brand} onChange={setBrand}/><label><span>Effective from</span><input type="datetime-local" value={from} onChange={(event) => setFrom(event.target.value)}/></label><label><span>Effective until</span><input type="datetime-local" value={until} onChange={(event) => setUntil(event.target.value)}/></label></div><div className="forms-task-actions"><Button variant="primary" isDisabled={busy || !bank.trim() || !locale.trim() || !from} isLoading={busy} onPress={() => void save()}>Save profile revision</Button></div></section>;
}
function ProfileLifecycle({ profile, busy, onTransition, onRollback }: { profile: CommunicationProfile; busy: boolean; onTransition: (to: CommunicationStatus) => Promise<void>; onRollback: () => Promise<void> }) { return <div className="forms-detail-actions">{profile.status === "DRAFT" && <Button variant="primary" isDisabled={busy} onPress={() => void onTransition("PENDING_APPROVAL")}>Send profile for approval</Button>}{profile.status === "PENDING_APPROVAL" && <Button variant="primary" isDisabled={busy} onPress={() => void onTransition("ACTIVE")}>Activate profile</Button>}{profile.status === "ACTIVE" && <Button isDisabled={busy} onPress={() => void onTransition("RETIRED")}>Retire profile</Button>}<Button isDisabled={busy} onPress={() => void onRollback()}>Create rollback draft</Button></div>; }
