...
export default function CommunicationsView() {
  ...
  async function serverPreview() {
    ...
    try { setPreview(await previewCommunicationTemplate(selected)); } catch (cause) { setError(message(cause, "Server preview could not be rendered.")); } finally { setBusy(undefined); }
  }
  ...
  async function testSend() {
    ...
    try { await testSendCommunicationTemplate(selected, testAddress.trim()); setNotice("Governed SMTP test send accepted by the server."); }
  ...
  return <section className="forms-task-shell" aria-labelledby="communications-title">
    <div className="forms-task-heading"><div><span>Governed delivery</span><h2 id="communications-title">Communications</h2><p>Profiles and message revisions are server-authoritative; preview, impact and test delivery never use local fake interpolation.</p></div><div className="forms-inline-actions"><button type="button" onClick={() => setProfileOpen(true)}>{profile ? "New profile revision" : "Create profile"}</button><button className="forms-primary" type="button" onClick={() => setEditorOpen(true)}>{selected ? "New template revision" : "Create template"}</button></div></div>
