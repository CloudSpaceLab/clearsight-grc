...
export const assessmentModes = [
  ...
  { id: "MANUAL", label: "Bank review" },
  { id: "AUTOMATIC", label: "Automatic rules" },
  { id: "AUTOMATIC_REVIEW", label: "Automatic rules, then bank review" },
...
export function assessmentConfigurationErrors(field: FormTemplateField): string[] {
  ...
  if (!needsBankReview(field) && (assessment.required || assessment.rubric?.length)) errors.push("Choose a bank review mode to require a review or rubric.");
  if (needsBankReview(field)) {
    ...
    if (!rubric.length || rubric.length > 50) errors.push("Add 1–50 bank rubric outcomes.");
