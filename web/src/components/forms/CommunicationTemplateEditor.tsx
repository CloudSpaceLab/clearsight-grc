...
import { $createListItemNode, $createListNode, $isListItemNode, $isListNode, ListItemNode, ListNode, INSERT_UNORDERED_LIST_COMMAND } from "@lexical/list";
...
function $createVariableNode(value: string) { const node = new VariableNode(value); node.setMode("token"); return node; }
...
function LexicalToolbar() {
  ...
  return <div className="forms-lexical-toolbar"><button type="button" onClick={() => editor.dispatchCommand(FORMAT_TEXT_COMMAND, "bold")}>Bold</button><button type="button" onClick={() => editor.dispatchCommand(FORMAT_TEXT_COMMAND, "italic")}>Italic</button><button type="button" onClick={() => editor.dispatchCommand(INSERT_UNORDERED_LIST_COMMAND, undefined)}>List</button><button type="button" onClick={() => editor.dispatchCommand(TOGGLE_LINK_COMMAND, "https://example.invalid/")}>Link</button></div>;
}
function VariableToolbar() {
  const [editor] = useLexicalComposerContext();
  return <div className="forms-variable-palette" aria-label="Protected communication variables">{variables.map((value) => <button type="button" key={value} onClick={() => editor.update(() => $insertNodes([$createVariableNode(`{{${value}}}`)]))}>{`{{${value}}}`}</button>)}</div>;
}
...
function importDocument(nodes: CommunicationNode[]) {
  ...
  for (const node of nodes) {
    ...
    if (type === "list") { const list = $createListNode("bullet"); for (const item of node.items ?? []) { const li = $createListItemNode(); appendProtectedText(li, item); list.append(li); } root.append(list); continue; }
    ...
    else { appendProtectedText(container, node.text ?? ""); if (type === "strong") container.getAllTextNodes().forEach((child) => child.toggleFormat("bold")); if (type === "emphasis") container.getAllTextNodes().forEach((child) => child.toggleFormat("italic")); }
