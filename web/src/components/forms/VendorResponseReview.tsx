  ...
  return <section className="vdd-response-application" aria-labelledby="response-application-title">
    ...
    <div className="vdd-comparison-list">{governed.map((answer) => {
      ...
      return <article key={answer.field_id} aria-label={`Apply decision: ${answer.label}`} className={conflict ? "has-conflict" : undefined}>
        ...
        <dl className="vdd-value-comparison"><div><dt>Requested held value</dt><dd>{answer.baseline!.display_value || "Not recorded"}<small>{answer.baseline!.source_label} · version {answer.baseline!.record_version}</small></dd></div><div><dt>Submitted response</dt><dd>{answerValue(answer)}<small>{assuranceLabel(answer)}</small></dd></div><div><dt>Current record</dt><dd>{currentHeldValue(answer, relationship)}<small>{identityTarget(answer) ? `Vendor version ${relationship.vendor.version}` : `Requested version ${answer.baseline!.record_version}`}</small></dd></div></dl>
