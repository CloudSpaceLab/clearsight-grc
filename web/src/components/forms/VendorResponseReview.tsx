...
export function VendorResponseReview({ relationship, review, onApply, onApplied }: Props) {
  ...
  if (receipt) return <section className="vdd-application-receipt" aria-labelledby="response-application-title"><span className="eyebrow">Application receipt</span><h3 id="response-application-title">Reviewed changes recorded</h3><p>{receipt.accepted_field_ids.length} accepted and {receipt.rejected_field_ids.length} rejected field decisions were recorded on {formatDateTime(receipt.applied_at)}.</p><dl><div><dt>Reviewer</dt><dd>{receipt.actor_display_name || "Reviewer name unavailable"}<details><summary>Review details</summary><p>Reviewer ID: {receipt.actor_principal_id}</p></details></dd></div><div><dt>Vendor version</dt><dd>{receipt.prior_vendor_version} → {receipt.result_vendor_version}</dd></div><div><dt>Assessment version</dt><dd>{receipt.result_assessment_version}</dd></div></dl></section>;
  ...
  return <section className="vdd-response-application" aria-labelledby="response-application-title">
    ...
    <div className="vdd-comparison-list">{governed.map((answer) => {
      ...
      return <article key={answer.field_id} aria-label={`Apply decision: ${answer.label}`} className={conflict ? "has-conflict" : undefined}>
        ...
        <dl className="vdd-value-comparison"><div><dt>Requested held value</dt><dd>{answer.baseline!.display_value || "Not recorded"}<small>{answer.baseline!.source_label} · version {answer.baseline!.record_version}</small></dd></div><div><dt>Submitted response</dt><dd><AnswerValueDisplay type={answer.type} answer={answer.value} emptyLabel="Not provided"/><small>{assuranceLabel(answer)}</small></dd></div><div><dt>Current record</dt><dd>{currentHeldValue(answer, relationship)}<small>{identityTarget(answer) ? `Vendor version ${relationship.vendor.version}` : `Requested version ${answer.baseline!.record_version}`}</small></dd></div></dl>
