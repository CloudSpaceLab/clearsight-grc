...
import { vendorWorkFilters, assessmentStateLabels } from "../vendorFormPresentation";
import { coverageText, scorePresentation } from "./forms/responseScorePresentation";
...
function FormsForRelationship({ relationshipID, serviceName, onRequestForm, onUpdated, onOpenHistory, onOpenDueDiligence, onOpenRequest, refreshKey, initialFilter }: Props) {
  ...
  return <section className="vendor-forms-panel" aria-label={`Forms and responses for ${serviceName}`}>
    ...
    <details className="vendor-request-details" open={initialFilter ? true : undefined}><summary>Requests{page ? ` · ${page.items.length} on this page` : ""}</summary>
    ...
    {page && <><p>{page.items.length} requests on this page · Checked <time dateTime={page.observed_at}>{dateTime(page.observed_at)}</time></p>
      ...
      <ul className="vendor-forms-list">{page.items.map((row) => {
        ...
        return <li key={`${row.request_id}:${row.response_id ?? "pending"}`}>
          ...
          {!submitted && !received && <p>Saved progress does not include unsaved browser input.</p>}
          ...
          <div className="vendor-forms-list__actions">{received && onOpenDueDiligence && <Button variant="primary" onPress={onOpenDueDiligence}>Review evidence</Button>}{submitted && row.response_id && <Button variant="primary" aria-label={`Review ${row.title} response`} onPress={() => setResponse(row)}>Review response</Button>}{submitted && row.response_id && <Button aria-label={`View ${row.title} documents`} onPress={() => setDocuments(row)}>View documents</Button>}{submitted && row.current && onOpenDueDiligence && row.attention_items?.some(item => item.state === "EXPIRED" && (item.kind === "VENDOR_DOCUMENT" || item.kind === "VENDOR_RESPONSE_FIELD")) && <Button onPress={onOpenDueDiligence}>Request updated items</Button>}{row.distribution_id && <Button onPress={() => setDistributionID(row.distribution_id)}>Manage request</Button>}{!row.distribution_id && onOpenRequest && <Button onPress={() => onOpenRequest(row.request_id)}>Open request</Button>}</div>
        </li>;
      })}</ul>
      {page.next_cursor && <Button isLoading={loadingMore} onPress={() => void more()}>Load more vendor forms</Button>}
    </>}
    </details>
    {response?.response_id && <FocusedSheet label={`Review ${response.title} response`} size="wide" onClose={() => setResponse(undefined)}><ResponseAssessment responseID={response.response_id} submissionScore={response.score} onUpdated={updated}/></FocusedSheet>}
...
function VendorFormScore({ title, score, provisional }: { title: string; score?: ResponseScore; provisional?: boolean }) {
  ...
  return <p><strong>{title}: </strong>{presentation.value}{score?.band ? ` · ${score.band.toLowerCase()} concern` : ""}{provisional || score?.state === "PROVISIONAL" ? " · Provisional" : ""} · Coverage: {coverageText(score)}</p>;
}
...
function assessmentState(value: string) { return assessmentStateLabels[value] ?? "Assessment state unavailable"; }
