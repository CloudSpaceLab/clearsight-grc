...
export function MatterFormRemediationPanel({ aggregate, operations, onUpdated, onOpenRequest, onMappingsChange }: Props) {
  ...
  return <article className="matter-record-panel matter-form-remediation-panel">
    <div className="matter-record-section-heading"><div><span className="eyebrow">Linked evidence</span><h2>Approved form requests</h2><p>Send an approved form for the outstanding items. Review the response before updating this issue.</p></div>
    ...
    {!loading && states.length === 0 && <p>No approved form has been sent for the outstanding items in this issue. {ownerCanAct ? "Send a form to collect the information." : "The current issue owner can send a form."}</p>}
    {states.length > 0 && <ul className="matter-form-remediation-list">{states.map((state) => <li key={state.binding.id}>
      <div><strong>{state.request?.title ?? `Form revision ${state.binding.form_template_version}`}</strong><span>{state.next_action} · {state.binding.mappings.length} outstanding item{state.binding.mappings.length === 1 ? "" : "s"} covered</span></div>
      ...
      {state.request && state.next_action === "Request correction" && <div className="matter-form-review-action"><Notice tone="warning">The response score is below the approved threshold. Review it or request a correction; this issue remains open.</Notice>{onOpenRequest && <Button variant="secondary" onPress={() => onOpenRequest(state.request!.id)}>Review response</Button>}</div>}
      {state.response && !state.application && reviewerCanAct && state.next_action === "Review evidence" && <div className="matter-form-review-action"><TextArea label="Review basis" value={rationaleByBinding[state.binding.id] ?? ""} onChange={(value) => setRationaleByBinding((current) => ({ ...current, [state.binding.id]: value }))} description="Explain why this response answers the outstanding items."/><Button variant="primary" isDisabled={busy || (rationaleByBinding[state.binding.id]?.trim().length ?? 0) < 20} onPress={() => void applyResponse(state)}>Apply response</Button></div>}
      {state.application && <span className="status-pill success">Information added · confirm the result before closing</span>}
    </li>)}</ul>}
    ...
    {creating && <FocusedSheet label="Send linked form" closeLabel={busy ? "Linked form is being sent" : "Close linked form setup"} size="wide" isDismissable={!busy} onClose={() => setCreating(false)}>
      <div className="cs-sheet-heading"><span className="eyebrow">Issue evidence</span><h2>Send an approved form</h2><p>Choose which form answer supplies each outstanding item. These choices are fixed after the request is sent.</p></div>
