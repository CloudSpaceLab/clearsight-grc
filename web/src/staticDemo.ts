...
const programOperations = {
  ...
  operations: [
    ...
    { command: "program.evidence.assess", label: "Record evidence check results", responsibility: "REVIEWER", can_act: false, assigned_to: { id: "role-dpco", display_name: "Data Protection Compliance Officer", kind: "POSITION", role: "DPCO reviewer" }, reason: "Assigned to Data Protection Compliance Officer for the current Program state." },
...
export async function staticDemoRequest<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (pathname === "/api/v1/programs" && method === "POST") {
    ...
    const createdProgram = { ...program, id: "program-created", code: input.code, name: input.name, type: input.type, status: "DRAFT", owning_function: input.owning_function, jurisdiction: input.jurisdiction, scope: input.scope ?? {}, effective_from: input.effective_from, version: 1 };
  ...
  if (pathname === `/api/v1/programs/${programID}/evidence-assessments` && method === "POST") {
    const input = requireProgramVersion(init); const detail = programDetail as any; detail.evidence_assessments.push({ id: `assessment-${detail.evidence_assessments.length + 1}`, ...input, contract_id: input.contract_id, assessed_at: input.assessed_at, valid_until: input.valid_until, assessed_by: "role-cro" }); finishProgramMutation(); return clone(detail) as T;
  }
