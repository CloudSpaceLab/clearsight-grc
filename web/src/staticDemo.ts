...
export async function staticDemoRequest<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (fixture === "today-unavailable" && pathname === "/api/v1/today") throw new StaticDemoHTTPError(503, "today_unavailable", "Assigned work is unavailable.");
  ...
  if (vendorWorkCommandMatch && method === "POST") {
    ...
    if (command === "send" || command === "retry") {
      ...
      outcome = { work: updated, state: updated.delivery_state, delivery: { status: partial ? "FAILED" : "DELIVERED" }, recovery: updated.recovery, ...(partial ? { capture_url: "https://capture.example.test/#form_access=sample-link" } : {}) };
