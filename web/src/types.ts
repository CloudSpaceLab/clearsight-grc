...
export type Readiness = { tenant_id: string; status: string; generated_at: string; dimensions: { current: number; aging: number; at_risk: number; unknown: number; blocked_routing: number; pending_human: number; }; active_drifts: Drift[]; recommended_actions: string[]; };
...
export type WorkflowTask = { id: string; tenant_id: string; workflow_id: string; step_key: string; responsibility: string; principal_id?: string; title: string; status: string; due_at?: string; context?: Record<string, string>; version: number; };
