...
describe("static stakeholder demo transport", () => {
  ...
  it("returns an onboarding guide for the requested workspace surface", async () => {
    ...
    const vendors = await staticDemoRequest<{ code: string; surface: string; role_codes: string[] }>("/api/v1/onboarding/guide?surface=VENDORS&code=vendor-operations-first-run");
