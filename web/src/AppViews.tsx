...
export function WorkView({ organizationName, actorPrincipalID, evidenceScopeToken, tab, onTab, onBackMatter, sources, requests, evidenceSourceState, evidenceRequestState, onEvidenceRetry, onEvidenceRequestUpdated, matterTargetID, openFirstMatter, evidenceTargetID, openFirstEvidence, onOpenEvidence }: { organizationName: string; actorPrincipalID?: string; evidenceScopeToken: number; tab: "matters" | "evidence"; onTab: (value: "matters" | "evidence") => void; onBackMatter: () => void; sources: EvidenceSource[]; requests: EvidenceRequest[]; evidenceSourceState: SectionLoadState; evidenceRequestState: SectionLoadState; onEvidenceRetry: () => void; onEvidenceRequestUpdated: (request: EvidenceRequest, scopeToken: number) => boolean; matterTargetID?: string; openFirstMatter?: boolean; evidenceTargetID?: string; openFirstEvidence?: boolean; onOpenEvidence: (id: string) => void }) {
  return <>
    ...
    {tab === "matters" ? <WorkspaceErrorBoundary label="Issues and changes"><MattersWorkspace targetID={matterTargetID} openFirst={openFirstMatter} onBack={onBackMatter}/></WorkspaceErrorBoundary> : <EvidenceWorkspace sources={sources} requests={requests} sourceState={evidenceSourceState} requestState={evidenceRequestState} actorPrincipalID={actorPrincipalID} evidenceScopeToken={evidenceScopeToken} targetID={evidenceTargetID} openFirst={openFirstEvidence} onOpenRequest={onOpenEvidence} onRequestUpdated={onEvidenceRequestUpdated}/>}
