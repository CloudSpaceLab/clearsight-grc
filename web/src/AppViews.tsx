...
export function ExploreView({ organizationName }: { organizationName: string }) {
  return <><header className="topbar"><div><span className="eyebrow">{organizationName}</span><h1>Explore</h1><p>Reference workflows across obligations, evidence, decisions, responses and confirmed outcomes.</p></div></header><BankJourneysWorkspace/></>;
}
...
export function ConfigureView({ policies, policyState, findings, integrityState, tasks, taskState, projectionHealth, projectionState, automationPolicies, automationPolicyState, state, onRetry, onReconcile }: { policies: PolicySummary[]; policyState: SectionLoadState; findings: IntegrityFinding[]; integrityState: SectionLoadState; tasks: WorkflowTask[]; taskState: SectionLoadState; projectionHealth: ProjectionHealth | null; projectionState: SectionLoadState; automationPolicies: AutomationPolicy[]; automationPolicyState: SectionLoadState; state: LoadState; onRetry: () => void; onReconcile: () => Promise<ReconcileResult> }) {
  ...
  if (state === "unavailable") return <div id="configure-workspace"><header className="topbar"><div><span className="eyebrow">Governance configuration</span><h1>Routing and approvals</h1><p>Responsibility, approval limits, delegation and escalation rules.</p></div></header><EmptyState kind="unavailable" label="Routing and approvals" title="Routing configuration could not be loaded" description="The service is unavailable or your current role cannot read this configuration. No policy or integrity claims are shown." action="Try again" onAction={onRetry}/></div>;
...
export function RoutingPanel({ resolution, item, legalEntityName, state }: { resolution: AuthorityResolution | null; item: AttentionItem | null; legalEntityName: string; state: RoutingLoadState }) {
  if (state === "loading") return <div className="panel-content"><span className="eyebrow">Authority</span><h2>Resolving current authority</h2><p aria-live="polite" aria-busy="true">Checking the exact record, responsibility and current policy…</p></div>;
  if (state === "forbidden") return <div className="panel-content"><EmptyState kind="forbidden" label="Authority" title="Authority details are restricted" description="Your current role cannot inspect this authority resolution. No candidate or policy details are shown."/></div>;
  ...
  if (state === "unavailable" || !resolution || !item) return <div className="panel-content"><EmptyState kind="unavailable" label="Authority" title="Authority resolution is unavailable" description="No authorizer, candidate set or approval sequence is inferred while resolution is unavailable."/></div>;
  const candidates = resolution.candidate_principals ?? [];
  const hasResolvedPrincipal = Boolean(resolution.principal?.id);
  return <div className="panel-content"><span className="eyebrow">Authority · {item.authority?.responsibility ?? "Current responsibility"}</span><h2>Authority for this item</h2><p><strong>{item.title}</strong></p><p>ClearSight resolved authority for the exact linked {item.action_target_type?.toLowerCase().replaceAll("_", " ")} record. Missing stages or limits are not inferred.</p>{hasResolvedPrincipal && <div className="resolution-card"><div className="principal-avatar">{initials(resolution.principal.display_name)}</div><div><strong>{resolution.principal.display_name}</strong><span>{resolution.principal.role} · {resolution.principal.kind}</span></div><mark>Resolved</mark></div>}{candidates.length > 1 && <section className="authority-candidates"><h3>Eligible candidates</h3><p>{resolution.strategy ? `Resolution strategy: ${humanizeStatus(resolution.strategy)}` : "The current policy returned more than one eligible principal."}</p><ul>{candidates.map((candidate) => <li key={candidate.id}><strong>{candidate.display_name}</strong><span>{candidate.role} · {candidate.kind}</span></li>)}</ul></section>}<dl className="explanation-list"><div><dt>Legal entity</dt><dd>{legalEntityName}</dd></div><div><dt>Responsibility</dt><dd>{item.authority?.responsibility ?? "Unavailable"}</dd></div>{item.authority?.decision_type && <div><dt>Decision type</dt><dd>{humanizeStatus(item.authority.decision_type)}</dd></div>}<div><dt>Materiality input</dt><dd>{item.authority ? String(item.authority.materiality) : "Unavailable"}</dd></div><div><dt>Policy version</dt><dd>{resolution.policy_version || "Unavailable"}</dd></div><div><dt>Resolution basis</dt><dd>{resolution.explanation || "No additional authority explanation was returned."}</dd></div></dl></div>;
}
