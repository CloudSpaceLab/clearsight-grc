...
describe("forms distribution API", () => {
  it("reads the canonical list projection and preserves server keyset filters", async () => {
    fetchMock.mockResolvedValueOnce(new Response(JSON.stringify({ items: [{ id: "dist-a", form_template_id: "form-a", form_template_version: 3, subject_type: "VENDOR", subject_id: "vendor-a", title: "Review", purpose: "Collect evidence", access_policy: "DIRECT_LINK_EMAIL_OTP", status: "OPEN", deadline: "2026-09-01T10:00:00Z", route_expires_at: "2026-09-01T09:00:00Z", version: 2, created_at: "2026-08-28T10:00:00Z", updated_at: "2026-08-28T11:00:00Z" }], next_cursor: "next" }), { status: 200, headers: { "Content-Type": "application/json" } }));
  ...
  it("pins the exact form revision when dispatching", async () => {
    fetchMock.mockResolvedValueOnce(new Response(JSON.stringify({ distribution: { id: "dist-a", form_template_id: "form-a", form_template_version: 7, subject_type: "CONTROL", subject_id: "c1", title: "Review", purpose: "Evidence", access_policy: "DIRECT_MAGIC_LINK", status: "OPEN", deadline: "2026-09-01T10:00:00Z", route_expires_at: "2026-09-01T09:00:00Z", version: 2, created_at: "2026-08-28T10:00:00Z", updated_at: "2026-08-28T10:00:00Z" }, recipients: [], workspace: { id: "w1", status: "OPEN", version: 1, updated_at: "2026-08-28T10:00:00Z" } }), { status: 201, headers: { "Content-Type": "application/json" } }));
