...
describe("reporting evidence transport", () => {
  it("serves the real reporting reads and records each handled URL", async () => {
    ...
    const fields = await json(await fetch("/api/v1/ropa/reports/filter-fields"));
    expect(fields.fields.some((field: { field: string }) => field.field === "name")).toBe(true);
    const definitions = await json(await fetch("/api/v1/ropa/reports/definitions?tenant_id=bank-demo"));
    ...
    const run = await json(await fetch(`/api/v1/ropa/reports/runs/${runID}?tenant_id=bank-demo`));
    expect(run.failure_code).toBe("row_limit_exceeded");

    expect((window as EvidenceWindow).reportingEvidenceReads).toEqual([
      "/api/v1/ropa/reports/filter-fields",
      "/api/v1/ropa/reports/definitions?tenant_id=bank-demo",
      `/api/v1/ropa/reports/definitions/${activeID}?tenant_id=bank-demo`,
      `/api/v1/ropa/reports/definitions/${activeID}/history?tenant_id=bank-demo`,
      "/api/v1/ropa/reports/runs?tenant_id=bank-demo&limit=50",
      `/api/v1/ropa/reports/runs/${runID}?tenant_id=bank-demo`,
    ]);
  ...
  it("normalizes Request inputs and returns the bounded-stop fixture for the named review state", async () => {
    ...
    const response = await fetch(new Request("https://example.test/api/v1/ropa/reports/runs?tenant_id=bank-demo&definition_id=report-active"));
  ...
  it("returns a not-found response for an unknown definition or run", async () => {
    ...
    const definition = await fetch("/api/v1/ropa/reports/definitions/missing?tenant_id=bank-demo");
    expect(definition.status).toBe(404);
    expect((await definition.json()).error.code).toBe("report_not_found");
    const run = await fetch("/api/v1/ropa/reports/runs/missing?tenant_id=bank-demo");
