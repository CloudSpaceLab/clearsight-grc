...
func seedFormPolicyAcceptance(
	...
	access, err := evidence.NewDistributionAccessService(
		distributionStore,
		keyring,
		nil,
		cfg.RecipientSecurity.AccessHMACKey,
	...
	now := time.Now().UTC()
	good, err := submitScoringAcceptanceResponse(
		ctx,
		distributions,
		access,
		seed,
		form,
		scoring.SubjectID,
		scoringResponseFixture{label: "post-policy-good", answers: scoringResponseFixtures[0].answers},
		now,
	)
	if err != nil {
		return formPolicyAcceptanceResult{}, fmt.Errorf("submit post-policy good response: %w", err)
	}
	poor, err := submitScoringAcceptanceResponse(
		ctx,
		distributions,
		access,
		seed,
		form,
		scoring.SubjectID,
		scoringResponseFixture{label: "post-policy-poor", answers: scoringResponseFixtures[2].answers},
		now.Add(time.Second),
	)
	if err != nil {
	...
	exactlyOnce, err := verifyExactlyOnceMatter(
		ctx,
		pool,
		seed.TenantID,
		seed.LegalEntityID,
