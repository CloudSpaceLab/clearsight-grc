...
func (reader seedFormReader) GetDistributionFormRevision(ctx context.Context, tenantID, legalEntityID, formID string, version int64) (evidence.DistributionFormRevision, error) {
...
func seedFormPolicyAcceptance(
	...
	if seed.ActorID == "" || seed.ContributorPrincipalID == "" || seed.ReviewerPrincipalID == "" || seed.SignatoryPrincipalID == "" {
		return formPolicyAcceptanceResult{}, fmt.Errorf("response-policy acceptance requires actor, contributor, reviewer and signatory principals")
	}
	...
	policyService.ConfigureActivationAuthority(formpolicy.ActivationAuthority{Automation: automation, Authority: authorityService, Subjects: evidence.CanonicalSubjectTypeRegistry{}})

	maker := formpolicy.Actor{TenantID: seed.TenantID, LegalEntityID: seed.LegalEntityID, PrincipalID: seed.ContributorPrincipalID}
	...
	base := formpolicy.CreateInput{
		...
		Eligibility: formpolicy.Eligibility{
			FormTemplateID: scoring.FormID, FormTemplateVersion: scoring.FormVersion,
			SubjectTypes: []string{"PROGRAM"}, CurrentOnly: true, MinimumCoverage: 1,
			Bands: []formcontract.ConcernBand{formcontract.ConcernHigh, formcontract.ConcernCritical},
		},
		Action: formpolicy.MatterAction{
			Type: "CONTROL_GAP", Priority: 4, TitleTemplate: "Review {{form_title}}",
	...
	if shadow.Status == formpolicy.PolicyActive {
		shadow, err = policyService.Suspend(ctx, authorizer, shadow.ID, shadow.RecordVersion)
		if err != nil {
	...
	firstPoor, err := executionForResponse(ctx, pool, enforce.ID, poor.ID)
	if err != nil {
		return formPolicyAcceptanceResult{}, err
	}
	firstGood, err := executionForResponse(ctx, pool, enforce.ID, good.ID)
	if err != nil {
		return formPolicyAcceptanceResult{}, err
	}
	if firstGood.State != string(formpolicy.ExecutionNotMatched) || firstGood.MatterID != "" || firstGood.CreatedMatter {
		return formPolicyAcceptanceResult{}, fmt.Errorf("good response created or matched a Matter: %#v", firstGood)
	}
	if firstPoor.State != string(formpolicy.ExecutionApplied) || firstPoor.MatterID == "" || !firstPoor.CreatedMatter {
		return formPolicyAcceptanceResult{}, fmt.Errorf("poor response did not create a governed Matter: %#v", firstPoor)
	}
	if err := publisher.Publish(ctx, poorEvent); err != nil {
	...
	var matterCount int
	if err := pool.QueryRow(ctx, `SELECT count(*) FROM matters WHERE tenant_id=$1::uuid AND id=$2::uuid AND source_type='FORM_RESPONSE'`, seed.TenantID, firstPoor.MatterID).Scan(&matterCount); err != nil {
		return formPolicyAcceptanceResult{}, fmt.Errorf("verify automatic Matter: %w", err)
	}
	return formPolicyAcceptanceResult{
		PolicyID: enforce.ID, PolicyVersion: enforce.Version,
		GoodResponseID: good.ID, PoorResponseID: poor.ID,
		GoodExecutionState: firstGood.State, PoorExecutionState: firstPoor.State,
		MatterID: firstPoor.MatterID, CreatedMatter: firstPoor.CreatedMatter,
		ReplayExecutionID: replayed.ID, ExactlyOnceMatter: matterCount == 1 && replayed.ID == firstPoor.ID && replayed.MatterID == firstPoor.MatterID,
	}, nil
}
...
func ensureAcceptanceAutomationPolicy(ctx context.Context, pool *pgxpool.Pool, seed bankverticals.SeedConfig, base formpolicy.CreateInput, rollout formpolicy.RolloutMode, version int64) (string, error) {
	policy := formpolicy.Policy{Eligibility: base.Eligibility, BlastRadius: base.BlastRadius, Outcome: base.Outcome, Rollout: rollout}
	...
	err := pool.QueryRow(ctx, `
		WITH t AS (SELECT id FROM tenants WHERE id::text=$1 OR slug=$1 LIMIT 1),
		     le AS (SELECT id,tenant_id FROM legal_entities WHERE (id::text=$2 OR code=$2) AND valid_until IS NULL LIMIT 1),
		     inserted AS (
		       INSERT INTO automation_policies(
		         id,tenant_id,code,name,action_class,eligibility,blast_radius_limit,verification_contract,status,effective_from,version,ai_definition,rollout_mode,maker_id,checker_id,checksum,approved_at,activated_at,record_version
		       )
		       SELECT md5('clearsight:'||t.id::text||':'||$3||':'||$4::text)::uuid,t.id,$3,$5,'FORM_RESPONSE_CREATE_MATTER',$6::jsonb,$7::jsonb,$8::jsonb,'ACTIVE',clock_timestamp()-interval '1 minute',$4,'{}'::jsonb,$9,$10::uuid,$11::uuid,encode(digest($3||':'||$4::text||':'||$9,'sha256'),'hex'),clock_timestamp(),clock_timestamp(),1
		       FROM t JOIN le ON le.tenant_id=t.id
		       ON CONFLICT DO NOTHING
		       RETURNING id
		     )
		SELECT id::text FROM inserted
		UNION ALL
		SELECT ap.id::text FROM automation_policies ap JOIN t ON t.id=ap.tenant_id WHERE ap.code=$3 AND ap.version=$4
		LIMIT 1`, seed.TenantID, seed.LegalEntityID, code, version, name, string(eligibility), string(blast), string(outcome), string(rollout), seed.ContributorPrincipalID, seed.ReviewerPrincipalID).Scan(&id)
	if err != nil {
...
func scoredOutboxEvent(ctx context.Context, pool *pgxpool.Pool, tenantID, responseRevisionID string) (workflowruntime.OutboxEvent, error) {
	...
	err := pool.QueryRow(ctx, `
		...
		FROM outbox_events
		WHERE tenant_id=$1::uuid AND event_type='FORM_RESPONSE_SCORED' AND payload->>'response_revision_id'=$2
		ORDER BY occurred_at DESC,id DESC LIMIT 1`, tenantID, responseRevisionID).Scan(&event.ID, &event.TenantID, &event.AggregateType, &event.AggregateID, &event.EventType, &event.Payload, &event.OccurredAt)
...
func executionForResponse(ctx context.Context, pool *pgxpool.Pool, policyID, responseRevisionID string) (storedExecution, error) {
	...
	err := pool.QueryRow(ctx, `
		...
		ORDER BY created_at DESC,id DESC LIMIT 1`, policyID, responseRevisionID).Scan(&value.ID, &value.State, &value.MatterID, &value.CreatedMatter)
	if err != nil {
...
func existingAcceptanceExecution(ctx context.Context, pool *pgxpool.Pool, seed bankverticals.SeedConfig) (formPolicyAcceptanceResult, bool, error) {
	...
	err := pool.QueryRow(ctx, `
		SELECT p.id::text,p.version,e.response_revision_id::text,e.state,COALESCE(e.matter_id::text,''),e.created_matter,e.id::text
		FROM form_response_policy_definitions p
		JOIN form_response_policy_executions e ON e.policy_id=p.id AND e.tenant_id=p.tenant_id AND e.legal_entity_id=p.legal_entity_id
		WHERE p.tenant_id=$1::uuid AND p.legal_entity_id=$2::uuid AND p.code=$3 AND p.rollout_mode='ENFORCE' AND p.status='ACTIVE' AND e.state IN ('APPLIED','REUSED')
		ORDER BY p.version DESC,e.created_at DESC LIMIT 1`, seed.TenantID, seed.LegalEntityID, responsePolicyAcceptanceCode).Scan(&result.PolicyID, &result.PolicyVersion, &result.PoorResponseID, &result.PoorExecutionState, &result.MatterID, &result.CreatedMatter, &result.ReplayExecutionID)
	if errors.Is(err, pgx.ErrNoRows) {
	...
	var matterCount int
	if result.MatterID != "" {
		if err := pool.QueryRow(ctx, `SELECT count(*) FROM matters WHERE tenant_id=$1::uuid AND id=$2::uuid AND source_type='FORM_RESPONSE'`, seed.TenantID, result.MatterID).Scan(&matterCount); err != nil {
			return formPolicyAcceptanceResult{}, false, err
		}
	}
	result.ExactlyOnceMatter = result.CreatedMatter && matterCount == 1
