...
const workClassMatterLifecycleProjection = "matter-lifecycle-work-projection"
...
func buildWorker(ctx context.Context, cfg config.Config, logger *slog.Logger) (workerSet, error) {
	...
	lifecycleWork := &workflow.MatterLifecycleProjector{
		Repo: workflowRepository, Continuity: continuityService,
	...
	service.AddMaintainerClass(workClassMatterLifecycleProjection, lifecycleWork)
