...
func main() {
	...
	gatewayOperations, err := buildAIGatewayOperations(cfg)
	if err != nil {
		logger.Error("AI gateway operations bridge initialization failed", "error", err)
		os.Exit(1)
	}
	gatewayPublicBaseURL, err := buildAIGatewayPublic(cfg)
	if err != nil {
		logger.Error("AI gateway public proxy configuration failed", "error", err)
		os.Exit(1)
	}
	handler := httpapi.New(httpapi.Dependencies{
		...
		Continuity:         services.Continuity, Ropa: services.Ropa, RopaEventsReader: services.RopaEventsReader, Reporting: services.Reporting, MatterFormRemediation: matterFormRemediation, Today: services.Today, Oversight: services.Oversight, Workflow: services.Workflow, Onboarding: services.Onboarding,
