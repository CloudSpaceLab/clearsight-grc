...
7 September extension: the user requested premium focused viewing, advanced search and sophisticated automated document-validation rulesets. The user subsequently rejected the mockup and directed actual implementation of a familiar macOS Finder-style browser with easy file-type filters and previews, with no more mockups. The actual shared browser and protected read APIs are implemented and independently reviewed; production release gates remain open. See the [implementation decision and rendered proof](../design/2026-09-07-submitted-document-browser.md) and [DOC-08–DOC-15 extension tracker](../superpowers/specs/2026-09-07-document-review-and-validation-design.md). Advanced validation remains future work, not a completed capability.
...
| DOC-16 | P1 | Implemented; release review tracked in PR #198 | `e98fcc38` denies titles/IDs/timestamps/scores for vendor work whose restricted target the actor cannot read, before pagination, while retaining authorized generic Forms access. Memory, HTTP and real-PostgreSQL tests cover mixed pages, exact reads, malformed links/request scope, reviewer routes and revocation. The raw-answer detail exposure was fixed separately in `14ab785f`. |
| DOC-17 | P1 | Implemented; follow-up release verification | Live full-page accessibility testing after PR #198 found the selected Documents tab referenced a stale panel ID. A shared one-line keyed-panel correction passed RED/GREEN regression testing, independent review and desktop/mobile full-page axe checks. The recurring CI document scenario now asserts the tab-to-panel link. No visual redesign was added. |
