...
### Live accessibility correction

The initial demo deployment (`144855d6`, PR #198) succeeded, but a full-page live axe scan after switching to Documents exposed a pre-existing shared Tabs identity defect: the selected tab referenced a panel ID that still belonged to Templates. Earlier scoped preview scans did not include the tab bar. A new unit regression reproduced the missing target before correction. Keying the existing panel by the selected tab renews its cached accessible identity; the tab list remains mounted, preserving keyboard focus. This does not add copy, tokens, layout variants or a new tab implementation. Activity/Audit mode-local filters now initialize to each mode's defaults when switching panels, consistent with separate views.

After correction, 17 affected UI/workflow/copy tests and 12 UI/scenario contracts passed, the evidence build passed, and full-page light-desktop/dark-mobile axe scans returned no violations. All ten document-browser viewport/theme checks passed again. Before/after screenshots remain in the release verification temporary directory. The recurring Documents scenario now also checks the selected tab's controlled panel ID, so this failure cannot hide behind a preview-only scan. The follow-up PR records CI, deployment and live re-verification results.
