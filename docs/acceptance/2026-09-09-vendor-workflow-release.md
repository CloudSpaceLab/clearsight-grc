...
- The actual deployment seed exposed a strict event-consumer mismatch for the new request ID/version provenance. The typed contract was corrected without removing producer metadata or accepting unknown fields. A fresh deployment seed and a repeat run passed every CI assertion, preserved response/issue identities and retained provenance on all six scored events. The form-policy PostgreSQL suite and full Go suite passed after this correction.
