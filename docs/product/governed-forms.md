...
Recognized XLSX findings registers produce vendor follow-up proposals with one numbered assessment selected per draft. Bank findings, recommendations, ratings and original dates remain explanatory context; recipients answer response, remediation, responsibility, proposed-date and evidence questions. The author confirms the source grouping before draft creation. Placeholder vendor names do not match vendor records automatically; the request sender must select the correct existing relationship. Partial or conflicting registers cannot produce follow-up drafts. Other document formats retain their existing conversion behavior. See [conversion decision and acceptance](../design/2026-09-08-register-follow-up.md).
