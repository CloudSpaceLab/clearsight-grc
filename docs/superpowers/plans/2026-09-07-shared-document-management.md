...
- 7 September release continuation: the requested target is the existing non-production demo, not a production bank deployment. Production scanner/storage/recovery/capacity gates remain open without preventing this demo release; pending/quarantined content remains blocked. DOC-16 is implemented in `e98fcc38`, reusing exact workflow membership and read-authority predicates for response summaries before pagination. Missing or wrong-entity submitted requests fail closed. Affected default suites, both vet modes, and serial real-PostgreSQL Evidence/ThirdParty/HTTP suites passed, including the 10,000-response bound and mixed-page regressions. Independent release reviews and CI/deployed-SHA results are tracked in [PR #198](https://github.com/CloudSpaceLab/clearsight-grc/pull/198).
- 7 September UI release review: repeated ten actual-component Forms/Vendors viewport/theme checks with no overflow, browser errors or scoped axe violations. Four document-browser journeys now run in the existing CI evidence suite (`aefe152a`); independent review corrected manifest classification and synchronized its capability contract. Full web suite again passed 153 files / 978 tests, production build passed, and pre-release hosted Forms desktop/mobile checks passed. No additional mockup, UI framework, navigation layer or fabricated lifecycle action was added. Read-only post-deployment checks and the exact released revision are recorded on PR #198.
