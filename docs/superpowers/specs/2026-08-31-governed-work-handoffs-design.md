...
The delivery ledger stores no recipient plaintext, message body or link token. The deep link contains only the Matter identifier and requires normal staff authentication. Missing contact data is a terminal **not deliverable** receipt that does not roll back the assignment. A failure known to occur before provider acceptance may claim one later attempt. SMTP offers no portable provider idempotency key, so a stranded **delivery started** claim is an unknown outcome that requires operator reconciliation and is never automatically resent. A disconnect after the SMTP server accepts DATA—including failure of the later QUIT exchange—is finalized as **delivery outcome unknown**, not temporary failure. This prevents a process crash or ambiguous acknowledgement after acceptance from producing duplicate mail. A recorded delivered, started, unknown or terminal receipt makes replay idempotent.
