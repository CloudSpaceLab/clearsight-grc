...
| `workflow_instances` | active projection | workflow | Matter Action projector | workflow projection and reads | one derived workflow per accountable subject where defined | rebuild only with source-domain reconciliation | `internal/workflow/matter_action_projector_postgres.go` |
...
| `org_positions` | active authoritative state | authority | governed organisation configuration and installers | authority resolver | valid interval | retain governed organisation history | `internal/authority/postgres.go` |
| `role_templates` | active authoritative state | authority | governed organisation configuration and installers | authority resolver and actor-role projection | valid interval | retain governed role history | authority resolver; actor-context tests |
| `position_role_bindings` | active authoritative state | authority | governed organisation configuration and installers | authority resolver | valid interval | retain governed binding history | `internal/authority/postgres.go` |
...
| `routing_policy_versions` | active authoritative state | governance | governance service | effective authority route projection | immutable version lineage with effective interval | retain approved/rejected versions | governance tests; migration 000014 triggers |
| `workflow_tasks` | active projection | workflow | Matter Action projector | Today and workflow reads | actor-facing status derived from accountable work | rebuild/reconcile from Matter Action truth | projector integration; Today tests |
| `workflow_events` | active projection | workflow | Matter Action projector | workflow projection history and diagnostics | append-only projection events | retain with projected workflow history | `internal/workflow/matter_action_projector_postgres.go`; projection integration |
...
| `capture_requests` | active authoritative state | evidence capture | evidence service | internal/external capture reads | draft/ready/in-progress/submitted/expired/cancelled with version | retain request/submission traceability | evidence PostgreSQL integration |
...
| `capture_invitations` | active authoritative state | evidence capture | evidence invitation service | redemption/revocation | issued/redeemed/revoked/expired | retain bounded invitation traceability | invitation/session tests |
| `capture_sessions` | active authoritative state | evidence capture | evidence session service | bounded external capture authorization | active/revoked/expired with last use | retain security/session traceability | session-guard/revocation tests |
