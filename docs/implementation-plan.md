...
Main advanced to the already deployed vendor-assessment release `11687656` during this work. Integration `3d3feeca` preserves both changes, with the sample-only index numbered 000085 after its three migrations. Fresh combined verification passed 160 web test files / 1,121 tests, TypeScript, customer-runtime fixture isolation and 36 UI contract checks. Full Go, PostgreSQL-tagged compilation/tests and vet passed after correcting a nil embedded store in an HTTP test fixture; no access check was removed. The earlier 224-state render receipt above predates this main integration and must not substitute for final combined UI evidence. Additional local full-Chromium light/dark renders displayed the actual one-page insurance PDF after its native loading indicator finished; the default headless-shell matrix still proves download fallback and exact bytes. The native readiness assertion, full final PostgreSQL/CI gates, hosted sample bytes and deployment remain pending under #138/#139/#147.
