...
The fictional-document demo exception is tested separately from live scanner acceptance. `internal/evidence/demo_sample_preview_test.go` checks every shipped asset with ordinary content admission, default refusal, exact name/media/digest/size eligibility, full stored-byte integrity, quarantine/deletion, unchanged scan state and AVAILABLE-only ordinary opening. It also checks that capability decoration copies and recomputes the authorized page, and that actual repository tenant/entity/principal/revision/field membership and revocation remain effective. `form_documents_test.go` proves both startup flags, fresh occurrence authorization, forged-flag rejection, changed bytes and protected delivery headers. `TestPostgresDemoSampleDocumentCapabilityRetainsExactScope` repeats scope and revocation checks through the PostgreSQL read model. Production configuration still rejects demo mode.

The shared document preview tests require **Demo check complete** and **No antivirus scan was performed** before demo content/download, protected PDF/image eligibility, Office fallback, no fetch/download for genuine pending or blocked states, and existing cancellation/length/media/retry cleanup. The [decision brief](../design/2026-09-08-demo-document-preview.md) and 26 focused `demo-document-*` browser scenarios cover both workspaces, desktop, 320px/390px, light/dark and keyboard return, including the shipped insurance PDF in both desktop themes. These tests do not satisfy deployed scanning, evidence review or hosted sample-installation acceptance.
...
7. Send two additional direct magic links with different expiry dates. Confirm exactly two new links were created and each of the three links remains usable until its own printed expiry or explicit revocation. Replace one email-OTP route and confirm its prior route is revoked. Request one targeted change and confirm the replacement capture retains the same request history.
