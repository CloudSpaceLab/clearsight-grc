SHELL := /bin/sh
.PHONY: check test test-postgres test-integration vet format run-api run-api-postgres run-worker run-worker-postgres run-ai-gateway run-ai-gateway-postgres web-install run-web compose-up compose-down
...
run-ai-gateway-postgres:
	go run -tags postgres ./cmd/ai-gateway
web-install:
