...
func (validator ExecutionAuthorityResolver) ResolvePolicyExecution(ctx context.Context, policy Policy, response evidence.CompletedResponseSummary) (ExecutionRoute, error) {
	...
	subjectOwner, err := resolveAuthority(ctx, validator.Authority, authority.ResolveInput{TenantID: policy.TenantID, LegalEntityID: policy.LegalEntityID, ObjectType: strings.ToUpper(scope.SubjectType), ObjectID: scope.SubjectID, Responsibility: authority.ResponsibilityOwner, DecisionType: "forms.response-policy.subject.resolve", Materiality: policy.Action.Priority})
	if err != nil {
	...
	owner, err := resolveAuthority(ctx, validator.Authority, authority.ResolveInput{TenantID: policy.TenantID, LegalEntityID: policy.LegalEntityID, ObjectType: "MATTER", ObjectID: "*", Responsibility: authority.ResponsibilityOwner, DecisionType: "matter.create", Materiality: policy.Action.Priority})
	if err != nil {
		return ExecutionRoute{}, err
	}
	ownerID := strings.TrimSpace(owner.Principal.ID)
	if ownerID == "" {
		return ExecutionRoute{}, ErrActivationAuthority
	}
	reviewer, err := resolveAuthority(ctx, validator.Authority, authority.ResolveInput{TenantID: policy.TenantID, LegalEntityID: policy.LegalEntityID, ObjectType: "MATTER", ObjectID: "*", Responsibility: authority.ResponsibilityReviewer, DecisionType: "matter.verify", Materiality: policy.Action.Priority})
	if err != nil {
		return ExecutionRoute{}, err
	}
	...
	performer, err := resolveAuthority(ctx, validator.Authority, authority.ResolveInput{TenantID: policy.TenantID, LegalEntityID: policy.LegalEntityID, ObjectType: "FORM_RESPONSE_POLICY", ObjectID: policy.ID, Responsibility: authority.ResponsibilityPerformer, DecisionType: "forms.response-policy.execute", Materiality: policy.Action.Priority})
	if err != nil {
		return ExecutionRoute{}, err
	}
	...
	return ExecutionRoute{TenantID: scope.TenantID, LegalEntityID: scope.LegalEntityID, CanonicalSubjectType: strings.ToUpper(scope.SubjectType), CanonicalSubjectID: scope.SubjectID, ServicePrincipalID: serviceID, OwnerPrincipalID: ownerID, ReviewerPrincipalID: reviewerID, ProgramID: programID}, nil
}
