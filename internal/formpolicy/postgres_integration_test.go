...
func seedPolicyResponse(t *testing.T, ctx context.Context, pool *pgxpool.Pool, now time.Time) {
	...
	if _, err := pool.Exec(ctx, `
		...
		VALUES(md5('form-policy:distribution:1')::uuid,$1::uuid,$2::uuid,$4::uuid,1,'VENDOR',md5('form-policy:subject:1')::uuid,'Vendor response','Review the completed response.','DIRECT_MAGIC_LINK','COMPLETED',$5::timestamptz+interval '30 days',$5::timestamptz+interval '7 days',$3::uuid,1,$5,$5);
