...
func (r *PostgresRepository) ApplyTriggerBundle(ctx context.Context, bundle TriggerBundle) (TriggerBundleResult, error) {
	...
	if err := tx.QueryRow(ctx, `SELECT p.version,p.legal_entity_id::text FROM programs p JOIN tenants t ON t.id=p.tenant_id WHERE (t.id::text=$1 OR t.slug=$1) AND p.id=$2::uuid AND p.legal_entity_id IS NOT NULL AND (NOT $3 OR ((t.id::text=$4 OR t.slug=$4) AND ($5='*' OR p.legal_entity_id=(SELECT le.id FROM legal_entities le WHERE le.tenant_id=p.tenant_id AND (le.id::text=$5 OR le.code=$5) AND le.valid_from<=clock_timestamp() AND (le.valid_until IS NULL OR clock_timestamp()<le.valid_until) ORDER BY le.valid_from DESC,le.id LIMIT 1)))) FOR UPDATE`, trigger.TenantID, trigger.ProgramID, enforce, actorTenant, actorEntity).Scan(&currentVersion, &programEntity); err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return TriggerBundleResult{}, ErrNotFound
		}
		return TriggerBundleResult{}, err
	}
	tag, err := tx.Exec(ctx, `INSERT INTO program_trigger_events(id,tenant_id,program_id,trigger_type,subject_type,subject_id,dedupe_key,payload,observed_at,source) VALUES($1::uuid,(SELECT id FROM tenants WHERE id::text=$2 OR slug=$2),$3::uuid,$4,$5,$6,$7,$8,$9,$10) ON CONFLICT(tenant_id,program_id,dedupe_key) DO NOTHING`, trigger.ID, trigger.TenantID, trigger.ProgramID, strings.ToUpper(trigger.Type), trigger.SubjectType, trigger.SubjectID, trigger.DedupeKey, rawJSON(trigger.Payload, `{}`), trigger.ObservedAt, trigger.Source)
	if err != nil {
		return TriggerBundleResult{}, err
	}
	if tag.RowsAffected() == 0 {
		var existing Matter
		err := tx.QueryRow(ctx, `SELECT m.id::text,t.slug,m.legal_entity_id::text,m.reference,m.matter_type,m.status,m.priority,m.title,m.summary,m.scope,m.source_type,COALESCE(m.source_id::text,''),m.trigger_type,COALESCE(m.trigger_id::text,''),m.trigger_key,m.known_facts,m.missing_facts,m.contradictions,COALESCE(m.owner_principal_id::text,''),m.required_authority,m.due_at,m.closed_at,m.closure_reason,m.reopen_count,m.created_at,m.updated_at,m.version FROM matters m JOIN tenants t ON t.id=m.tenant_id JOIN matter_links ml ON ml.tenant_id=m.tenant_id AND ml.matter_id=m.id AND ml.retired_at IS NULL WHERE (t.id::text=$1 OR t.slug=$1) AND m.trigger_key=$2 AND ml.program_id=$3::uuid AND m.legal_entity_id=$4::uuid AND m.status NOT IN ('CLOSED','CANCELLED') ORDER BY m.created_at DESC LIMIT 1`, trigger.TenantID, trigger.DedupeKey, trigger.ProgramID, programEntity).Scan(&existing.ID, &existing.TenantID, &existing.LegalEntityID, &existing.Reference, &existing.Type, &existing.Status, &existing.Priority, &existing.Title, &existing.Summary, &existing.Scope, &existing.SourceType, &existing.SourceID, &existing.TriggerType, &existing.TriggerID, &existing.TriggerKey, &existing.KnownFacts, &existing.MissingFacts, &existing.Contradictions, &existing.OwnerPrincipalID, &existing.RequiredAuthority, &existing.DueAt, &existing.ClosedAt, &existing.ClosureReason, &existing.ReopenCount, &existing.CreatedAt, &existing.UpdatedAt, &existing.Version)
		if errors.Is(err, pgx.ErrNoRows) {
