...
func TestCommittedProgramReturnsCommandResultWhenRefreshReadFails(t *testing.T) {
	...
	program, err := service.CreateProgram(context.Background(), CreateProgramInput{TenantID: "bank", Code: "TPRM", Name: "Third-party risk", Type: "THIRD_PARTY", OwningFunction: "Risk", Scope: json.RawMessage(`{}`), EffectiveFrom: now})
	if err != nil {
...
func TestCommittedMatterLinkReturnsCommandResultWhenCurrentReadFails(t *testing.T) {
	...
	now := time.Date(2026, 8, 26, 12, 0, 0, 0, time.UTC)
	service.now = func() time.Time { return now }
	program, err := repository.MemoryRepository.CreateProgram(context.Background(), Program{ID: "program-1", TenantID: "bank", Code: "TPRM", Name: "Third-party risk", Type: "THIRD_PARTY", Status: ProgramDraft, OwningFunction: "Risk", Scope: json.RawMessage(`{}`), EffectiveFrom: now, CreatedAt: now, UpdatedAt: now, Version: 1}, Event{ID: "event-1", TenantID: "bank", AggregateType: "PROGRAM", AggregateID: "program-1", AggregateVersion: 1, Type: EventProgramCreated, Payload: json.RawMessage(`{}`), OccurredAt: now})
	if err != nil {
		t.Fatal(err)
	}
	matter, err := service.CreateMatter(context.Background(), CreateMatterInput{TenantID: "bank", Type: MatterControlGap, Priority: 3, Title: "Obtain vendor assurance", Summary: "The vendor must provide current assurance evidence.", Scope: json.RawMessage(`{}`), ProgramID: program.ID})
	if err != nil {
