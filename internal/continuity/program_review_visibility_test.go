...
func TestProgramReviewDigestIgnoresHiddenMatterOnlyProjectionChange(t *testing.T) {
	...
	program := Program{ID: "program-1", TenantID: "bank", Code: "REVIEW", Name: "Review visibility", Type: "COMPLIANCE", Status: ProgramActive, OwningFunction: "Compliance", Scope: json.RawMessage(`{}`), EffectiveFrom: t1, CreatedAt: t1, UpdatedAt: t1, Version: 1}
	...
	for _, principalID := range []string{"person-a", "person-b"} {
		if _, err := repo.RecordProgramReview(ctx, ProgramReviewCheckpoint{ID: "checkpoint-" + principalID, TenantID: "bank", ProgramID: program.ID, PrincipalID: principalID, ProgramVersion: 1, ProjectionVersion: 1, AcceptedAt: t1}); err != nil {
			t.Fatal(err)
		}
	}

	matter := Matter{ID: "matter-1", TenantID: "bank", Reference: "MAT-1", Type: MatterAuthorityRequest, Status: MatterAssessment, Priority: 4, Title: "Restricted issue", Summary: "Restricted issue", Scope: json.RawMessage(`{"access":"RESTRICTED","allowed_principal_ids":["person-b"]}`), KnownFacts: json.RawMessage(`{}`), MissingFacts: json.RawMessage(`[]`), Contradictions: json.RawMessage(`[]`), CreatedAt: t2, UpdatedAt: t2, Version: 1}
