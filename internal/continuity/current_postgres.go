...
SELECT jsonb_build_object(
  'program', (to_jsonb(p)-'tenant_id'-'program_type') || jsonb_build_object('tenant_id',t.slug,'type',p.program_type),
  'requirements', COALESCE((SELECT jsonb_agg((to_jsonb(v)-'tenant_id'-'object_name') || jsonb_build_object('tenant_id',t.slug,'object',v.object_name) ORDER BY v.created_at,v.id) FROM program_requirements v WHERE v.tenant_id=p.tenant_id AND v.program_id=p.id),'[]'::jsonb),
  'applicability', COALESCE((SELECT jsonb_agg((to_jsonb(v)-'tenant_id') || jsonb_build_object('tenant_id',t.slug) ORDER BY v.created_at,v.id) FROM program_applicability v WHERE v.tenant_id=p.tenant_id AND v.program_id=p.id),'[]'::jsonb),
  'control_objectives', COALESCE((SELECT jsonb_agg((to_jsonb(v)-'tenant_id') || jsonb_build_object('tenant_id',t.slug) ORDER BY v.created_at,v.id) FROM control_objectives v WHERE v.tenant_id=p.tenant_id AND v.program_id=p.id),'[]'::jsonb),
  'control_implementations', COALESCE((SELECT jsonb_agg((to_jsonb(v)-'tenant_id') || jsonb_build_object('tenant_id',t.slug) ORDER BY v.created_at,v.id) FROM control_implementations v WHERE v.tenant_id=p.tenant_id AND v.program_id=p.id),'[]'::jsonb),
  'requirement_control_links', COALESCE((SELECT jsonb_agg((to_jsonb(v)-'tenant_id') || jsonb_build_object('tenant_id',t.slug) ORDER BY v.created_at,v.id) FROM requirement_control_links v WHERE v.tenant_id=p.tenant_id AND v.program_id=p.id),'[]'::jsonb),
...
SELECT jsonb_build_object(
  'matter', (to_jsonb(m)-'tenant_id'-'matter_type') || jsonb_build_object('tenant_id',t.slug,'type',m.matter_type),
  'links', COALESCE((SELECT jsonb_agg((to_jsonb(v)-'tenant_id') || jsonb_build_object('tenant_id',t.slug) ORDER BY v.created_at,v.id) FROM matter_links v WHERE v.tenant_id=m.tenant_id AND v.matter_id=m.id),'[]'::jsonb),
  'decisions', COALESCE((SELECT jsonb_agg((to_jsonb(v)-'tenant_id'-'decision_type'-'matter_version') || jsonb_build_object('tenant_id',t.slug,'type',v.decision_type) ORDER BY v.matter_version,v.id) FROM matter_decisions v WHERE v.tenant_id=m.tenant_id AND v.matter_id=m.id),'[]'::jsonb),
  'actions', COALESCE((SELECT jsonb_agg((to_jsonb(v)-'tenant_id') || jsonb_build_object('tenant_id',t.slug) ORDER BY v.created_at,v.id) FROM matter_actions v WHERE v.tenant_id=m.tenant_id AND v.matter_id=m.id),'[]'::jsonb),
  'verification_contracts', COALESCE((SELECT jsonb_agg((to_jsonb(v)-'tenant_id') || jsonb_build_object('tenant_id',t.slug) ORDER BY v.created_at,v.id) FROM verification_contracts v WHERE v.tenant_id=m.tenant_id AND v.matter_id=m.id),'[]'::jsonb),
  'verification_results', COALESCE((SELECT jsonb_agg((to_jsonb(v)-'tenant_id') || jsonb_build_object('tenant_id',t.slug) ORDER BY v.observed_at,v.id) FROM verification_results v WHERE v.tenant_id=m.tenant_id AND v.matter_id=m.id),'[]'::jsonb),
  'response_packages', COALESCE((SELECT jsonb_agg((to_jsonb(v)-'tenant_id'-'matter_version') || jsonb_build_object('tenant_id',t.slug) ORDER BY v.matter_version,v.id) FROM response_packages v WHERE v.tenant_id=m.tenant_id AND v.matter_id=m.id),'[]'::jsonb)
)
FROM matters m
JOIN tenants t ON t.id=m.tenant_id
