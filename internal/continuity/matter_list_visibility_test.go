...
func TestMemoryMatterListFiltersBeforeLimit(t *testing.T) {
	...
	repo.matters["bank-a"] = map[string]MatterAggregate{
		...
		"visible": {Matter: Matter{ID: "visible", TenantID: "bank-a", Reference: "MAT-002", Type: MatterRegulatoryChange, Status: MatterAssessment, Priority: 4, Title: "Visible", Summary: "Internal", Scope: json.RawMessage(`{"access":"INTERNAL"}`), CreatedAt: now.Add(-time.Minute), UpdatedAt: now.Add(-time.Minute)}},
	}
	ctx := identity.WithActor(context.Background(), identity.Actor{TenantID: "bank-a", PrincipalID: "person-a"})
