...
func (r *PostgresRepository) ListProgramSummaries(ctx context.Context, tenant string, query SummaryQuery) (ProgramSummaryPage, error) {
	...
	rows, err := r.pool.Query(ctx, `
		...
		WHERE (t.id::text=$1 OR t.slug=$1)
		  AND (NOT $9 OR t.id::text=$11 OR t.slug=$11)
		  AND ($2='' OR p.status=$2)
		...
		LIMIT $8`, tenant, query.Status, query.Search, hasCursor, cursor.Rank, cursor.UpdatedAt, cursor.ID, limit+1, enforceVisibility, principalID, actorTenant)
	if err != nil {
		return ProgramSummaryPage{}, err
	}
	defer rows.Close()
