...
func (r *PostgresRepository) CreateRelationship(ctx context.Context, record CreateRecord) (Aggregate, error) {
	...
	if record.ReuseVendor {
		...
		err = tx.QueryRow(ctx, `
			INSERT INTO third_parties(id,tenant_id,legal_name,trading_name,registration_ref,jurisdiction,source_id,external_ref,registered_address,website_domain,status,created_at,updated_at,version)
			VALUES($1::uuid,$2::uuid,$3,$4,$5,$6,$7,$8,NULLIF($9,''),NULLIF($10,''),$11,$12,$12,1)
			ON CONFLICT (tenant_id,source_id,external_ref) WHERE source_id<>'' AND external_ref<>''
			DO UPDATE SET source_id=EXCLUDED.source_id
			RETURNING id::text`, record.Vendor.ID, tenantID, record.Vendor.LegalName, record.Vendor.TradingName, record.Vendor.RegistrationRef, record.Vendor.Jurisdiction, record.Vendor.SourceID, record.Vendor.ExternalRef, record.Vendor.RegisteredAddress, record.Vendor.WebsiteDomain, record.Vendor.Status, record.Vendor.CreatedAt).Scan(&vendorID)
		createdVendor = vendorID == record.Vendor.ID
	} else {
		_, err = tx.Exec(ctx, `
			INSERT INTO third_parties(id,tenant_id,legal_name,trading_name,registration_ref,jurisdiction,source_id,external_ref,registered_address,website_domain,status,created_at,updated_at,version)
			VALUES($1::uuid,$2::uuid,$3,$4,$5,$6,'','',NULLIF($7,''),NULLIF($8,''),$9,$10,$10,1)`, record.Vendor.ID, tenantID, record.Vendor.LegalName, record.Vendor.TradingName, record.Vendor.RegistrationRef, record.Vendor.Jurisdiction, record.Vendor.RegisteredAddress, record.Vendor.WebsiteDomain, record.Vendor.Status, record.Vendor.CreatedAt)
	...
	err = tx.QueryRow(ctx, `
		SELECT id::text,legal_name,trading_name,registration_ref,jurisdiction,source_id,external_ref,COALESCE(registered_address,''),COALESCE(website_domain,''),status,created_at,updated_at,version
		...
		&storedVendor.SourceID, &storedVendor.ExternalRef, &storedVendor.RegisteredAddress, &storedVendor.WebsiteDomain, &storedVendor.Status, &storedVendor.CreatedAt, &storedVendor.UpdatedAt, &storedVendor.Version,
	)
	if err != nil {
...
const relationshipSelect = `
	SELECT p.id::text,t.slug,p.legal_name,p.trading_name,p.registration_ref,p.jurisdiction,p.source_id,p.external_ref,COALESCE(p.registered_address,''),COALESCE(p.website_domain,''),p.status,p.created_at,p.updated_at,p.version,
...
func scanAggregate(row rowScanner) (Aggregate, error) {
	...
	err := row.Scan(
		...
		&value.Vendor.SourceID, &value.Vendor.ExternalRef, &value.Vendor.RegisteredAddress, &value.Vendor.WebsiteDomain, &value.Vendor.Status, &value.Vendor.CreatedAt, &value.Vendor.UpdatedAt, &value.Vendor.Version,
