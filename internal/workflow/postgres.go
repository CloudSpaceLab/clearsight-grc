...
func scanTask(row scanner) (Task, error) {
	var task Task
	var contextJSON []byte
	if err := row.Scan(&task.ID, &task.TenantID, &task.WorkflowID, &task.StepKey, &task.Responsibility, &task.PrincipalID, &task.Title, &task.Status, &task.DueAt, &contextJSON, &task.Version, &task.CreatedAt, &task.UpdatedAt); err != nil {
		return Task{}, err
	}
	_ = json.Unmarshal(contextJSON, &task.Context)
	return task, nil
