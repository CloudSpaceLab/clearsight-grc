...
func NewMemoryRepository(seed []Task) *MemoryRepository {
	...
	for _, task := range seed { task.Context = clone(task.Context); tasks[task.ID] = task }
...
func (r *MemoryRepository) Create(_ context.Context, input CreateInput) (Task, error) {
	...
	r.mu.Lock(); r.tasks[task.ID] = task; r.mu.Unlock()
	return task, nil
}
func (r *MemoryRepository) Get(_ context.Context, id string) (Task, error) {
	r.mu.RLock(); defer r.mu.RUnlock()
	task, ok := r.tasks[id]
	if !ok { return Task{}, ErrTaskNotFound }
...
func (r *MemoryRepository) List(_ context.Context, filter ListFilter) ([]Task, error) {
	...
	for _, task := range r.tasks {
		...
		if filter.PrincipalID != "" && task.PrincipalID != filter.PrincipalID { continue }
	...
	if len(values) > filter.Limit { values = values[:filter.Limit] }
	return values, nil
}
func (r *MemoryRepository) Transition(_ context.Context, id string, input TransitionInput) (Task, error) {
	...
	task, ok := r.tasks[id]
	if !ok { return Task{}, ErrTaskNotFound }
	if task.Version != input.ExpectedVersion { return Task{}, ErrVersionConflict }
	task.Status = input.Status; task.Version++; task.UpdatedAt = r.now().UTC(); r.tasks[id] = task
	return task, nil
}
func clone(input map[string]string) map[string]string { out := map[string]string{}; for key, value := range input { out[key] = value }; return out }
func DemoTasks() []Task {
	now := time.Now().UTC()
