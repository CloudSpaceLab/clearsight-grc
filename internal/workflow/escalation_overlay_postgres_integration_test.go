...
func TestEscalationOverlayDoesNotSurviveMaterialRoutingChange(t *testing.T) {
	...
	if _, err := tx.Exec(ctx, `INSERT INTO tenants(id,slug,name) VALUES($1::uuid,'overlay-semantic-test','Overlay semantic test')`, tenantID); err != nil {
		t.Fatal(err)
	}
	...
	if err := tx.QueryRow(ctx, `SELECT responsibility,status,COALESCE(context->>'escalation_active',''),COALESCE(context->>'materiality','') FROM workflow_tasks WHERE id=$1::uuid`, taskID).
