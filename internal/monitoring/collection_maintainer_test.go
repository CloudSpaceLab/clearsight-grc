...
func TestCollectionMaintainerReusesSuccessorAfterInterruption(t *testing.T) {
	...
	for _, request := range requests {
		if request.Origin != nil && request.Origin.ID == fixture.check.ID && request.Origin.Sequence == 2 {
			count++
		}
	}
	if count != 1 {
...
func TestCollectionMaintainerStopsAfterRecipientReassignmentIsRequired(t *testing.T) {
	...
	if _, err := fixture.evidence.DeclareWrongRecipient(ctx, evidence.DeclareWrongRecipientInput{TenantID: "bank-a", RequestID: request.ID, ActorPrincipalID: "respondent-1", Reason: "The vendor owner changed.", ExpectedVersion: request.Version}); err != nil {
		t.Fatal(err)
	}
	_, _, reminders := CollectionDates(fixture.submission.SubmittedAt, fixture.cycle.Policy)
...
func newRenewalFixture(t *testing.T, reminders int, routes ...RecipientRoute) renewalFixture {
	...
	form := FormTemplate{
		ID: "form-1", TenantID: "bank-a", Code: "VENDOR-SECURITY", Name: "Vendor security review", Purpose: "Collect the current vendor security response.",
	...
	predecessor, err := evidenceService.CreateRequest(ctx, evidence.CreateRequestInput{
		TenantID: "bank-a", SubjectType: "PROGRAM", SubjectID: check.ProgramID, Title: form.Name, Purpose: form.Purpose,
		...
		FormTemplateID: form.ID, FormTemplateVersion: form.Version, Origin: &evidence.RequestOrigin{Type: evidence.OriginMonitoringCollection, ID: check.ID, Sequence: 1}, CreatedBy: "owner-1",
	})
	if err != nil {
		t.Fatal(err)
	}
	receipt, err := evidenceRepo.Submit(ctx, evidence.Submission{ID: "submission-1", TenantID: "bank-a", RequestID: predecessor.ID, SubmittedBy: "respondent-1", Channel: "INTERNAL", Answers: map[string]string{"answer": "Confirmed"}, ExpectedVersion: predecessor.Version, SubmittedAt: submittedAt})
	if err != nil {
		t.Fatal(err)
	}
