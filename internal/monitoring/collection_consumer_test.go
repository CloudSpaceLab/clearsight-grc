...
func TestCollectionConsumerProjectsSubmissionOnce(t *testing.T) {
	...
	request, err := evidenceService.CreateRequest(ctx, evidence.CreateRequestInput{
		...
		Origin: &evidence.RequestOrigin{Type: evidence.OriginMonitoringCollection, ID: check.ID, Sequence: 1}, CreatedBy: "owner-1",
	})
	if err != nil {
		t.Fatal(err)
	}
	receipt, err := evidenceRepo.Submit(ctx, evidence.Submission{ID: "submission-1", TenantID: request.TenantID, RequestID: request.ID, SubmittedBy: "respondent-1", Channel: "INTERNAL", Answers: map[string]string{"answer": "Confirmed"}, ExpectedVersion: request.Version, SubmittedAt: submittedAt})
	if err != nil {
		t.Fatal(err)
	}
...
func TestCollectionConsumerCancelsOpenCycleWhenCheckIsRetired(t *testing.T) {
	...
	request := evidence.Request{ID: "request-2", TenantID: "bank-a", SubjectType: "PROGRAM", SubjectID: retired.ProgramID, Origin: &evidence.RequestOrigin{Type: evidence.OriginMonitoringCollection, ID: retired.ID, Sequence: 2}}
