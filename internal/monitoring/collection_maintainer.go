...
func (m *CollectionMaintainer) processClaim(ctx context.Context, claim CollectionCycle, now time.Time) error {
	...
	if request.Origin == nil || request.Origin.Type != evidence.OriginMonitoringCollection || request.Origin.ID != claim.MonitoringCheckID {
		return fmt.Errorf("collection request origin does not match the cycle")
	}
	if request.Origin.Sequence == claim.Sequence {
		return m.openRenewal(ctx, claim, request, now)
	}
	if request.Origin.Sequence == claim.Sequence+1 {
...
func (m *CollectionMaintainer) openRenewal(ctx context.Context, claim CollectionCycle, predecessor evidence.Request, now time.Time) error {
	...
	form, err := m.Repository.FormRevision(ctx, claim.TenantID, check.FormTemplateID, check.FormTemplateVersion)
	...
	input := evidence.CreateRequestInput{
		TenantID: claim.TenantID, SubjectType: "PROGRAM", SubjectID: claim.ProgramID, Title: form.Name, Purpose: form.Purpose,
		WhyYou: predecessor.WhyYou, Sensitivity: predecessor.Sensitivity, AudienceType: predecessor.AudienceType,
		EstimatedMinutes: estimateMinutes(len(fields)), Deadline: claim.ExpiresAt, KnownFacts: copyStringMap(predecessor.KnownFacts), Fields: fields,
		...
		Origin: &evidence.RequestOrigin{Type: evidence.OriginMonitoringCollection, ID: check.ID, Sequence: claim.Sequence + 1}, PredecessorRequestID: predecessor.ID,
...
func evidenceFields(fields []TemplateField) []evidence.Field {
	...
	for index, field := range fields {
		result[index] = evidence.Field{ID: field.ID, Label: field.Label, Type: field.Type, Required: field.Required, Description: field.Description, Options: append([]string(nil), field.Options...), AcceptedFormats: append([]string(nil), field.AcceptedFormats...)}
	}
	return result
}
