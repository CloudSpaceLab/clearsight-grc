...
func formProposalSourceKey(value FormTemplateProposal) string {
	return fmt.Sprintf("%s\x00%s\x00%s\x00%s\x00%d\x00%s\x00%s\x00%d\x00%s",
		...
		strings.TrimSpace(value.BaseTemplateID), value.BaseTemplateVersion, value.FindingAssessmentID)
}
