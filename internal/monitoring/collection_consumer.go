...
func (c *CollectionConsumer) Publish(ctx context.Context, event workflowruntime.OutboxEvent) error {
	...
	if request.Origin == nil || request.Origin.Type != evidence.OriginMonitoringCollection {
	...
	if _, err := c.Repository.CompleteCollectionCyclesBeforeSequence(ctx, event.TenantID, check.ID, request.Origin.Sequence, now); err != nil {
	...
	cycle := CollectionCycle{
		...
		Sequence: request.Origin.Sequence, Policy: policy, CurrentRequestID: request.ID, PredecessorRequestID: request.PredecessorRequestID,
