...
func TestMemoryRepositoryActivatingReplacementCheckRetiresCurrentCheckWithSameProgramCode(t *testing.T) {
	...
	activated, err := repo.TransitionCheck(t.Context(), LifecycleTransition{TenantID: "bank-a", ID: replacement.ID, ExpectedVersion: 2, To: LifecycleActive, ActorID: "reviewer", At: now.Add(time.Minute)})
	if err != nil {
		t.Fatal(err)
	}
	...
	retired, err := repo.CheckRevision(t.Context(), "bank-a", current.ID, current.Version)
	if err != nil {
		t.Fatal(err)
	}
