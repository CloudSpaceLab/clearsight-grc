...
func TestPostgresConcurrentReplacementApprovalBindsCurrentAndRecordsRetirement(t *testing.T) {
	...
	ids := make([]string, 8)
	for index := range ids {
		ids[index], err = platformid.NewUUIDv7()
		if err != nil {
			t.Fatal(err)
		}
	}
	tenantID, entityID, programID, currentID, firstID, secondID, actorID, bindingID := ids[0], ids[1], ids[2], ids[3], ids[4], ids[5], ids[6], ids[7]
	tenantSlug := "monitoring-concurrency-" + tenantID
	defer cleanupMonitoringConcurrencyTenant(context.Background(), pool, tenantID)
	now := time.Date(2026, 9, 3, 8, 0, 0, 0, time.UTC)
	if _, err = pool.Exec(ctx, `
		INSERT INTO tenants(id,slug,name) VALUES($1::uuid,$6,'Monitoring Concurrency Test');
		...
		VALUES($3::uuid,$1::uuid,$2::uuid,'MON-CONCURRENT','Monitoring concurrency','COMPLIANCE','ACTIVE','Compliance','NG','{}'::jsonb,$5)`, pgx.QueryExecModeSimpleProtocol, tenantID, entityID, programID, actorID, now, tenantSlug); err != nil {
		t.Fatal(err)
	}
	repo := NewPostgresRepository(pool)
	base := MonitoringCheck{ID: currentID, TenantID: tenantSlug, ProgramID: programID, Code: "ENCRYPTION", Name: "Encryption check", Claim: "Encryption remains enabled.", InputKind: InputSource, BindingID: bindingID, BindingVersion: 1, SourceRules: []SourceRule{{ID: "enabled", Field: "enabled", Operator: OperatorEquals, Expected: "true", RiskPoints: 100}}, Thresholds: DefaultThresholds(), FreshnessMinutes: 60, MinimumCoverage: 1, FailureAction: FailureReview, Lifecycle: Lifecycle{Status: LifecyclePaused, IsCurrent: true, Version: 3, EffectiveFrom: &now, CreatedAt: now, UpdatedAt: now}}
	first := base
	first.ID = firstID
...
func cleanupMonitoringConcurrencyTenant(ctx context.Context, pool *pgxpool.Pool, tenantID string) {
	...
	_, _ = pool.Exec(ctx, `DELETE FROM monitoring_checks WHERE tenant_id=$1::uuid`, tenantID)
