...
func TestExecutionGrantRequiresExactApprovedMatterDecisionAndIsSingleUse(t *testing.T) {
	...
	matter, err := matters.CreateMatter(ctx, continuity.CreateMatterInput{TenantID: "bank", Type: continuity.MatterAuthorityRequest, Priority: 5, Title: "Approve high-impact AI action", Summary: "Approve one exact AI tool action.", Scope: json.RawMessage(`{}`)})
	if err != nil {
		t.Fatal(err)
	}
