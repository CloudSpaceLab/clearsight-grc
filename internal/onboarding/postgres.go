...
func (r *PostgresRepository) Get(ctx context.Context, tenant, principal, guide string) (State, error) {
	...
	err := r.pool.QueryRow(ctx, `
		...
		tenant, principal, guide,
	).Scan(&value.TenantID, &value.PrincipalID, &value.GuideCode, &value.GuideVersion, &value.CurrentStep, &value.Completed, &value.Dismissed, &value.UpdatedAt, &value.Version)
...
func (r *PostgresRepository) Upsert(ctx context.Context, value State, expected int64) (State, error) {
	var result State
	err := r.pool.QueryRow(ctx, `
		INSERT INTO user_onboarding_state(tenant_id,principal_id,guide_code,guide_version,current_step,completed_at,dismissed_at,version)
		SELECT t.id,$2::uuid,$3,$4,$5,
		       CASE WHEN $6 THEN clock_timestamp() END,
		       CASE WHEN $7 THEN clock_timestamp() END,1
		FROM tenants t
		WHERE (t.id::text=$1 OR t.slug=$1) AND $8=0
		ON CONFLICT(tenant_id,principal_id,guide_code) DO UPDATE
	...
	if err != nil {
		return State{}, fmt.Errorf("save onboarding state: %w", err)
	}
	return result, nil
}
