...
func TestInternalRecipientOwnsQueueAndAuthenticatedSubmission(t *testing.T) {
	...
	receipt, err := service.Submit(ctx, Submission{TenantID: "bank", RequestID: request.ID, SubmittedBy: "actor-a", Channel: "MAGIC_LINK", Answers: map[string]string{"owner": "Operations"}, ExpectedVersion: 1})
	if err != nil {
		t.Fatal(err)
	}
...
func TestExternalInvitationMustMatchCanonicalAudience(t *testing.T) {
	...
	_, err = service.IssueInvitation(ctx, IssueInvitationInput{TenantID: "bank", RequestID: request.ID, Audience: "other@example.com", Purpose: "Respond", TTLMinutes: 30})
	if !errors.Is(err, ErrRecipientMismatch) {
		t.Fatalf("invitation audience was allowed to drift from canonical request recipient: %v", err)
	}
	issued, err := service.IssueInvitation(ctx, IssueInvitationInput{TenantID: "bank", RequestID: request.ID, Audience: "customer@example.com", Purpose: "Respond", TTLMinutes: 30})
	if err != nil {
		t.Fatal(err)
	}
