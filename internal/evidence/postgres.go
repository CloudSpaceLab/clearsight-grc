...
func (r *PostgresRepository) ListRequests(ctx context.Context, tenant string, limit int) ([]Request, error) {
	rows, err := r.pool.Query(ctx, `SELECT `+requestSelectColumns+` FROM capture_requests er JOIN tenants t ON t.id=er.tenant_id WHERE (t.id::text=$1 OR t.slug=$1) ORDER BY er.deadline,er.id LIMIT $2`, tenant, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *PostgresRepository) GetRequest(ctx context.Context, tenant, id string) (Request, error) {
	value, err := scanRequest(r.pool.QueryRow(ctx, `SELECT `+requestSelectColumns+` FROM capture_requests er JOIN tenants t ON t.id=er.tenant_id WHERE er.id=$1::uuid AND (t.id::text=$2 OR t.slug=$2)`, id, tenant))
	if errors.Is(err, pgx.ErrNoRows) {
...
func (r *PostgresRepository) Submit(ctx context.Context, submission Submission) (SubmissionReceipt, error) {
	...
	defer tx.Rollback(ctx)
	request, err := scanRequest(tx.QueryRow(ctx, `SELECT `+requestSelectColumns+` FROM capture_requests er JOIN tenants t ON t.id=er.tenant_id WHERE er.id=$1::uuid AND (t.id::text=$2 OR t.slug=$2) FOR UPDATE`, submission.RequestID, submission.TenantID))
	if errors.Is(err, pgx.ErrNoRows) {
...
func (r *PostgresRepository) CreateInvitation(ctx context.Context, value Invitation) error {
	...
	defer tx.Rollback(ctx)
	request, err := scanRequest(tx.QueryRow(ctx, `SELECT `+requestSelectColumns+` FROM capture_requests er JOIN tenants t ON t.id=er.tenant_id WHERE er.id=$1::uuid AND (t.id::text=$2 OR t.slug=$2) FOR UPDATE`, value.RequestID, value.TenantID))
	if errors.Is(err, pgx.ErrNoRows) {
		return ErrNotFound
	}
	if err != nil {
