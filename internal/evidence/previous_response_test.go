...
func TestBuildPreviousResponsePrefillCopiesOnlyCompatibleScalars(t *testing.T) {
	...
	submission := Submission{ID: "submission-1", RequestID: previous.ID, SubmittedAt: submittedAt, Answers: formcontract.TextAnswers(map[string]string{
...
func TestBuildPreviousResponsePrefillRejectsChangedTypeAndOptions(t *testing.T) {
	...
	submission := Submission{ID: "submission-1", RequestID: previous.ID, SubmittedAt: time.Date(2026, 8, 14, 10, 32, 0, 0, time.UTC), Answers: formcontract.TextAnswers(map[string]string{
...
func TestPreviousResponseProvenanceTracksConfirmationAndCorrection(t *testing.T) {
	...
	got := service.deriveAnswerProvenance(context.Background(), request, formcontract.TextAnswers(map[string]string{"same": previous.Value, "changed": "Acme Payments Limited"}))
...
func TestPreviousResponseProvenanceDefersToCurrentGovernedSource(t *testing.T) {
	...
	got := service.deriveAnswerProvenance(context.Background(), request, formcontract.TextAnswers(map[string]string{"branch": "Current branch"}))["branch"]
...
func TestPreviousResponseLineageMustMatchImmutableSubmission(t *testing.T) {
	...
	firstInput.Origin = RequestOrigin{Type: OriginMonitoringCollection, ID: "11111111-1111-7111-8111-111111111111", Version: 1}
	...
	receipt, err := service.Submit(context.Background(), Submission{
		...
		ExpectedVersion: previous.Version, Answers: formcontract.TextAnswers(map[string]string{"answer": "Confirmed"}),
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	nextInput.Origin = RequestOrigin{Type: OriginMonitoringCollection, ID: firstInput.Origin.ID, Version: 2}
