...
func (service *DistributionAccessService) ResponseRecoveryContext(ctx context.Context, session DistributionAccessSession) (DistributionRecoveryContext, error) {
	if service == nil || strings.TrimSpace(session.TenantID) == "" || strings.TrimSpace(session.LegalEntityID) == "" || strings.TrimSpace(session.DistributionID) == "" {
	...
	if err != nil || !distributionOpenForAccess(bundle.Distribution, service.currentTime()) || bundle.Distribution.LegalEntityID != session.LegalEntityID || bundle.Distribution.FormTemplateVersion < 1 || bundle.Distribution.RouteExpiresAt.IsZero() {
		return DistributionRecoveryContext{}, ErrSessionInvalid
	}
