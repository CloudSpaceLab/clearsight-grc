...
func (s *Service) seedAuthorityRequest(ctx context.Context, config SeedConfig, program continuity.ProgramAggregate, sourceID string) (continuity.MatterAggregate, error) {
	...
	request, err := s.evidence.CreateRequest(ctx, evidence.CreateRequestInput{TenantID: config.TenantID, SubjectType: "MATTER", SubjectID: matter.Matter.ID, Title: "Provide incident containment and customer communication records", Purpose: "Complete the restricted NDPC response package.", WhyYou: "You own the incident response records requested by Regulatory Affairs.", Sensitivity: "RESTRICTED", AudienceType: "INTERNAL", EstimatedMinutes: 10, Deadline: config.Now.Add(48 * time.Hour), KnownFacts: map[string]string{"case_reference": "NDPC/ENF/2026/0142", "incident_reference": "PRI-2026-008"}, Fields: []evidence.Field{{ID: "containment_record", Label: "Containment record", Type: "FILE", Required: true, AcceptedFormats: []string{"application/pdf", "text/csv"}}, {ID: "communication_decision", Label: "Customer communication decision", Type: "TEXT", Required: true, Description: "State the approved decision and approving authority."}}, CreatedBy: config.ActorID})
	if err != nil {
		return matter, err
	}
