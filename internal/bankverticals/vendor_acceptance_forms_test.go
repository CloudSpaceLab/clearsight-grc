...
func TestVendorCertificationRefreshFormContract(t *testing.T) {
	...
	if len(form.Sections) != 3 || len(form.Fields) != 5 {
		t.Fatalf("certification form sections=%d fields=%d", len(form.Sections), len(form.Fields))
	}
	for _, pair := range []struct{ applicability, document string }{{"iso_applicable", "iso_certificate"}, {"pci_applicable", "pci_attestation"}} {
		var applicability, document *formcontract.Field
