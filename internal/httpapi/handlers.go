...
func (a *API) ready(w http.ResponseWriter, _ *http.Request) { httpx.WriteJSON(w, http.StatusOK, map[string]string{"status": "ready", "mode": a.deps.Mode}) }
...
func (a *API) simulateAuthority(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { httpx.WriteError(w, http.StatusInternalServerError, "simulation_failed", "Routing could not be simulated."); return }
...
func (a *API) authorityIntegrity(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { httpx.WriteError(w, http.StatusBadRequest, "integrity_failed", err.Error()); return }
...
func (a *API) authorityPolicies(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { httpx.WriteError(w, http.StatusBadRequest, "policies_failed", err.Error()); return }
...
func (a *API) listWorkflowTasks(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { httpx.WriteError(w, http.StatusInternalServerError, "workflow_failed", "Tasks could not be loaded."); return }
	httpx.WriteJSON(w, http.StatusOK, map[string]any{"items": values})
}
func (a *API) createWorkflowTask(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { httpx.WriteError(w, http.StatusUnprocessableEntity, "workflow_invalid", err.Error()); return }
...
func (a *API) transitionWorkflowTask(w http.ResponseWriter, r *http.Request) {
	...
	switch {
	case errors.Is(err, workflow.ErrTaskNotFound): httpx.WriteError(w, http.StatusNotFound, "not_found", "Task not found.")
	case errors.Is(err, workflow.ErrVersionConflict): httpx.WriteError(w, http.StatusConflict, "version_conflict", "Task changed. Reload before updating.")
	case errors.Is(err, workflow.ErrInvalidTransition): httpx.WriteError(w, http.StatusUnprocessableEntity, "invalid_transition", "The requested transition is not allowed.")
	case err != nil: httpx.WriteError(w, http.StatusInternalServerError, "workflow_failed", "Task could not be updated.")
	default: httpx.WriteJSON(w, http.StatusOK, value)
	}
}
func (a *API) onboardingGuide(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { httpx.WriteError(w, http.StatusNotFound, "not_found", "Guide not found."); return }
...
func (a *API) onboardingState(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { httpx.WriteError(w, http.StatusBadRequest, "state_failed", err.Error()); return }
...
func (a *API) updateOnboardingState(w http.ResponseWriter, r *http.Request) {
	...
	if errors.Is(err, onboarding.ErrVersionConflict) { httpx.WriteError(w, http.StatusConflict, "version_conflict", "Onboarding state changed. Reload before updating."); return }
	if err != nil { httpx.WriteError(w, http.StatusUnprocessableEntity, "state_failed", err.Error()); return }
	httpx.WriteJSON(w, http.StatusOK, value)
}
func (a *API) readiness(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { httpx.WriteError(w, http.StatusBadRequest, "readiness_failed", err.Error()); return }
	httpx.WriteJSON(w, http.StatusOK, value)
}
func (a *API) ingestSignal(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { httpx.WriteError(w, http.StatusUnprocessableEntity, "signal_failed", err.Error()); return }
