...
func TestSourceCatalogCreateConnectionUsesVerifiedActorScopeAndRemainsListable(t *testing.T) {
	...
	body := `{"code":"CORE_BANKING","name":"Core banking","adapter_kind":"POSTGRES","adapter_version":"postgres-v1","secret_ref":"vault://bank-a/source-readers/core-banking","declared_capabilities":["INSPECT","PAGE"]}`
	...
	listRequest := httptest.NewRequest(http.MethodGet, "/api/v1/config/sources/"+sourceID+"/connections", nil)
	listRequest.SetPathValue("source_id", sourceID)
	listRequest = listRequest.WithContext(identity.WithActor(listRequest.Context(), identity.Actor{TenantID: tenantID, PrincipalID: principalID, ExpiresAt: time.Now().Add(time.Hour)}))
	listResponse := httptest.NewRecorder()
	api.listSourceConnections(listResponse, listRequest)
	if listResponse.Code != http.StatusOK {
		t.Fatalf("list status=%d body=%s", listResponse.Code, listResponse.Body.String())
	}
	var listed struct {
		Items []sourceaccess.ConnectionRevision `json:"items"`
	}
	if err := json.Unmarshal(listResponse.Body.Bytes(), &listed); err != nil {
		t.Fatal(err)
	}
	if len(listed.Items) != 1 || listed.Items[0].ConnectionID != created.ConnectionID || listed.Items[0].Status != sourceaccess.RevisionDraft {
		t.Fatalf("created draft disappeared after refresh: %#v", listed.Items)
	}
}
