...
func (a *API) listFormDistributions(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { writeFormDistributionError(w, err); return }
...
func (a *API) createFormDistribution(w http.ResponseWriter, r *http.Request) {
	...
	if !ok { return }
	bundle, err := service.Create(r.Context(), evidence.CreateDistributionInput{
		TenantID: actor.TenantID, LegalEntityID: legalEntityID,
...
func (a *API) amendFormDistribution(w http.ResponseWriter, r *http.Request) {
	...
	value, err := service.Amend(r.Context(), actor.TenantID, legalEntityID, r.PathValue("id"), evidence.AmendDistributionInput{
		ExpectedVersion: request.ExpectedVersion, Deadline: request.Deadline, RouteExpiresAt: request.RouteExpiresAt, ReminderPolicy: request.ReminderPolicy,
	})
	if err != nil { writeFormDistributionError(w, err); return }
...
func (a *API) rotateFormDistributionAccessRoute(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { writeFormAccessAdminError(w, err); return }
	secureNoStore(w)
...
// Supersession is intentionally exposed now so clients have one stable route;
// the exact-revision carry-forward transaction is implemented by the lifecycle
// service before this PR leaves draft state.
...
func (a *API) startFormAccess(w http.ResponseWriter, r *http.Request) {
	...
	if err := httpx.DecodeJSON(w, r, &request); err != nil { writeGenericFormAccessFailure(w); return }
	value, err := access.StartDistributionAccess(r.Context(), request.RouteSelector)
	if err != nil { writeGenericFormAccessFailure(w); return }
...
func (a *API) getFormResponseWorkspace(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { writeGenericFormSessionFailure(w); return }
...
func (a *API) saveFormResponseWorkspace(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if errors.As(err, &conflict) { httpx.WriteJSON(w, http.StatusConflict, conflict); return }
		writeGenericFormSessionFailure(w); return
	}
	httpx.WriteJSON(w, http.StatusOK, value)
}
...
func (a *API) distributionMutationContext(w http.ResponseWriter, r *http.Request) (*evidence.DistributionService, identity.Actor, string, bool) {
	service, ok := a.formDistributionService(w)
	if !ok { return nil, identity.Actor{}, "", false }
	actor, ok := distributionActor(w, r)
	if !ok { return nil, identity.Actor{}, "", false }
	legalEntityID, ok := distributionLegalEntity(w, r, actor, r.URL.Query().Get("legal_entity_id"))
	if !ok { return nil, identity.Actor{}, "", false }
...
func distributionBearerToken(r *http.Request) string {
	...
	if len(header) < 8 || !strings.EqualFold(header[:7], "Bearer ") { return "" }
...
func distributionBundleJSON(bundle evidence.DistributionBundle) map[string]any {
	return map[string]any{
		"distribution": map[string]any{
			"id": bundle.Distribution.ID, "tenant_id": bundle.Distribution.TenantID, "legal_entity_id": bundle.Distribution.LegalEntityID,
			"form_template_id": bundle.Distribution.FormTemplateID, "form_template_version": bundle.Distribution.FormTemplateVersion,
			"subject_type": bundle.Distribution.SubjectType, "subject_id": bundle.Distribution.SubjectID,
			"title": bundle.Distribution.Title, "purpose": bundle.Distribution.Purpose, "access_policy": bundle.Distribution.AccessPolicy,
			"status": bundle.Distribution.Status, "deadline": bundle.Distribution.Deadline, "route_expires_at": bundle.Distribution.RouteExpiresAt,
			"reminder_policy": bundle.Distribution.ReminderPolicy, "created_by": bundle.Distribution.CreatedBy,
			"version": bundle.Distribution.Version, "created_at": bundle.Distribution.CreatedAt, "updated_at": bundle.Distribution.UpdatedAt,
		},
