...
func (a *API) routes() []routeSpec {
	routes := []routeSpec{
		...
		withPermission(material("/api/v1/third-party-activation-policies/{id}/approve", thirdparty.ActivationPolicyApproveCommand, a.approveThirdPartyActivationPolicy, commandPolicy{ObjectType: "THIRD_PARTY_ACTIVATION_POLICY", ObjectIDPath: "id", Responsibility: authority.ResponsibilityAuthorizer, Materiality: 4, ActorField: noActorField}), identity.PermissionConfigWrite),
